			...
			<div class="card text-left mt-5" style="width:100%; margin:0 auto;">
				<div class=" card-header">
					<div class=" row">
						...
						<div class="col">
							...
							<a href="downloadpollingstationinformation" target="_blank" class="btn show pdfbut"><img src="/assets/images/pdf.png" style="width: 53px !important;"></a>
				...
				<div class="card-body">
                    ...
                    <thead>
                        ...
                        <th colspan="3">General Electors</th>
                    ...
                    <tbody>
						...
						$gen_m_sum_tot  = $gen_f_sum_tot =  $ser_m_sum_tot = $ser_f_sum_tot = $pollingregaux_tot = 0;
						$flag = $stcode = $gen_m_sum  = $gen_f_sum =  $ser_m_sum = $ser_f_sum = $pollingregaux = 0;
						?>
						...
						<tr>
                            ...
                            <th>{{ $gen_m_sum }}</th>
                            <th>{{ $gen_f_sum }}</th>
							<th>{{ $gen_m_sum+$gen_f_sum }}</th>
                            <th>{{ $ser_m_sum }}</th>
                            <th>{{ $ser_f_sum }}</th>
							<th>{{ $ser_m_sum+$ser_f_sum }}</th>
                            <th>{{ $gen_m_sum+$gen_f_sum }}</th>
                            <th>{{ $ser_m_sum+$ser_f_sum }}</th>
                           
                            <th>{{ $gen_m_sum+$gen_f_sum+$ser_m_sum+$ser_f_sum }}</th>
                        </tr> 
							<?php 
							$gen_m_sum  = $gen_f_sum =  $ser_m_sum = $ser_f_sum = $pollingregaux = 0;
						...
						@if($stcode=='' || $stcode!=$pollingstation->st_code)
						 <tr>
                            <th colspan="13" class="gry">{{ $pollingstation->st_name }}</th>
                        </tr>
						...
						@endif
						 <tr>
                            ...
                            <td>{{ $pollingstation->total_no_polling_station }}</td>
                            <td>{{ $pollingstation->e_gen_m }}</td>
                            <td>{{ $pollingstation->e_gen_f }}</td>
							<td>{{ $pollingstation->e_gen_m+$pollingstation->e_gen_f }}</td>
                            <td>{{ $pollingstation->e_ser_m }}</td>
                            <td>{{ $pollingstation->e_ser_f }}</td>
							<td>{{ $pollingstation->e_ser_m+$pollingstation->e_ser_f }}</td>
                            <td>{{ $pollingstation->e_gen_m+$pollingstation->e_ser_m }}</td>
                            <td>{{ $pollingstation->e_gen_f+$pollingstation->e_ser_f }}</td>
                           
                            <td>{{ $pollingstation->e_gen_m+$pollingstation->e_ser_m+$pollingstation->e_gen_f+$pollingstation->e_ser_f }}</td>
                        </tr>
                       ...
                       <tr>
                        ...
                        <tr>
							...
							<td><b>{{$gen_m_sum_tot+$gen_f_sum_tot}}</b></td>
                            <td><b>{{$ser_m_sum_tot}}</b></td>
                            <td><b>{{$ser_f_sum_tot}}</b></td>
							<td><b>{{$ser_m_sum_tot+$ser_f_sum_tot}}</b></td>
							<td><b>{{$gen_m_sum_tot+$gen_f_sum_tot}}</b></td>
                            <td><b>{{$ser_m_sum_tot+$ser_f_sum_tot}}</b></td>
							<td><b>{{$gen_m_sum_tot+$gen_f_sum_tot+$ser_m_sum_tot+$ser_f_sum_tot}}</b></td>
                        </tr>
                    </tbody>
