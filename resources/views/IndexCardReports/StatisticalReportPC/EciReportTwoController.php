...
SUM(postal_vote) AS 'postal_vote',SUM(tended_votes) AS 'tended_votes', SUM(nota_vote) AS 'nota_vote',
SUM(postal_votes_rejected) AS 'postal_votes_rejected',SUM(test_votes_49_ma) AS 'test_votes_49_ma',
SUM(votes_not_retreived_from_evm) AS 'votes_not_retreived_from_evm',SUM(lead_total_vote) AS 'lead_total_vote',SUM(voters) AS
 'voters'
FROM (
SELECT TEMP1.*,ecoi.postal_votes_rejected,ecoi.test_votes_49_ma,ecoi.votes_not_retreived_from_evm,
SUM(ecoi.general_male_voters+ecoi.general_female_voters+ecoi.general_other_voters+ecoi.nri_male_voters+
ecoi.nri_female_voters+ecoi.nri_other_voters+service_postal_votes_under_section_8+service_postal_votes_gov) AS 'voters'
FROM
(
SELECT TEMP.*,SUM(cpm.evm_vote) AS 'evm_vote',SUM(cpm.postal_vote) AS 'postal_vote',SUM(cpm.tended_votes) AS 'tended_votes' , nota.nota_vote,wlc.lead_total_vote
FROM (
...
FROM counting_pcmaster f_nota, m_pc f_mpc WHERE f_mpc.st_code = f_nota.st_code AND f_mpc.pc_no = f_nota.pc_no AND f_nota.candidate_id = 4319
