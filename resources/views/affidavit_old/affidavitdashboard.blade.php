                            ...
                            <div class="row">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col">
                                        <div class="" style="width: 100%;">
                                            @if(!empty($getData))
                                            <select onkeypress="return blockSpecialChar(event)" name="ac_name" class="consttype form-control aclist" id="pc_no">
                                               ...
                                               <option value="{{@$raw->PC_NO}}" <?php if(@$raw->PC_NO == $getData->pc_no) echo 'selected="selected"'; ?>>{{@$raw->PC_NO}}-{{ @$raw->PC_NAME }} - {{ @$raw->PC_NAME_HI }}</option>											   
											   @endforeach
											...
											<option value="">-{{Lang::get('affidavit.select_ac_name')}}-</option>
												@php $acList = getpcbystate($session_data->st_code); @endphp
												@foreach($acList as $raw)
                                               <option value="{{@$raw->PC_NO}}" <?php if(@$raw->PC_NO == $session_data->pc_no) echo 'selected="selected"'; ?>>{{@$raw->PC_NO}}-{{ @$raw->PC_NAME }} - {{ @$raw->PC_NAME_HI }}</option>											   
											   @endforeach
    ...
    $.ajax({
        ...
        beforeSend: function(){
            ...
            $.each(response.result,function(index, item) {
                ...
                //     items+="<option selected='selected' value='"+item.AC_NO+"'>"+item.AC_NAME+"</option>";
                // } else {
                    items+="<option value='"+item.PC_NO+"'>"+item.PC_NO+"-"+item.PC_NAME+"-"+item.PC_NAME_HI+"</option>";    
                //}
