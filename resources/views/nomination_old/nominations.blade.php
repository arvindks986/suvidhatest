...
function doCopy(st, pc){
	...
	$.ajax({
		...
		success: function(msg){  
			if(msg >= 4){ 
				$("#copyload").hide();
				//$("#copy_nomination_"+id).hide();
				$('#already4Filled').modal('show');
				$('#copy_nomination_'+id).modal('hide');
				return false;
			} else {
            ...
            <div class="tab-content">
              <div id="submtd" class="tab-pane active">
				<?php $i=0; $k=1;  
					...
					$acdd = decrypt_String($_REQUEST['pcs']);
				...
				?>	
				   ...
				   <?php   $iddataa = 0;
					...
					$passac=encrypt_string($resd['pc_no']);
					?>
					<option @if(($resd['st_code']==$stdd) && ($resd['pc_no']==$acdd)){{'selected'}}@endif value="{{$passac.'***+++'.$passst}}">{{$resd['state']}}-{{$resd['pc_name']}}</option>
					@endforeach
					@endif
					</select>
					</div>	
				...
				<?php //echo "<pre>"; print_r($result);
						...
						<div class="modal-dialog modal-dialog-centered modal-dialog-zoom">
						  <div class="modal-content">
						   <div class="pop-header pt-3 pb-1">
							...
							<button type="submit" class="btn dark-purple-btn" onclick="return doCopy('<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');">{{ __('nomination.Yes') }}</button>
							</div>
						  ...
						  <div class="modal-content">
						   <div class="pop-header pt-3 pb-1">
							...
							<div class="header-caption">
							 ...
							 <div class="confirm-footer">
							  <button type="button" class="btn dark-pink-btn" data-dismiss="modal">{{ __('nomination.Cancel') }}</button>&nbsp;&nbsp;&nbsp;
							<button type="submit" class="btn dark-purple-btn" onclick="return doDelete('<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');">{{ __('nomination.Yes') }}</button>
							</div>
					...
					@if($iddataa==0)	
						...
						@if($result['finalize_after_payment']!=1)	
								...
								<span id="Delete_<?php echo $result['id']; ?>" onclick="return deleteNomination( '<?php echo $result['id']; ?>',  '<?php echo $result['nomination_no']; ?>', '<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');"><i class="fa fa-trash-o" aria-hidden="true"></i>
								{{ __('messages.Delete') }} 
								</span> 
								@endif	
								
								
								<span onclick="return copyNomination( '<?php echo $result['id']; ?>',  '<?php echo $result['nomination_no']; ?>', '<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');"><i class="fa fa-clone" aria-hidden="true"></i>
								{{ __('messages.copnom') }}
								</span>	
						...
						$isChallanSubmitted = app(App\Http\Controllers\Nomination\NominationController::class)->getChallan($result['st_code'], $result['pc_no']);
						?>
					...
					@endif
                        ...
                        <ul>
                          ...
                          <li><strong>{{ __('nomination.ac') }}. &amp; {{ __('nomination.Name') }}</strong> <span>{{$result['pc_name']}}</span></li>
						...
						<?php $stst = app(App\Http\Controllers\Nomination\NominationController::class)->getSchStatus($result['st_code'], $result['pc_no'] ); ?>
						
                        <div class="nomin-foot">
                          <div class="custom-control custom-checkbox">                            
							<!--<input type="checkbox"  class="custom-control-input cls_{{$result['id']}}" id="customCheck-{{$k}}" name="is_scrutiny_completed" value="{{$result['id']}}" onclick="return setRadio('<?php echo $result['id']; ?>', '<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');">-->
						@if($stst==0)
							
						<!--<label class="" for="" onclick="return bookAnAppointment('<?php echo $result['id']; ?>', '<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');">SCHEDULE APPOINTMENT</label> -->  
                    ...
                    @if($result['finalize_after_payment']!=1)		
							...
							<span id="Delete_<?php echo $result['id']; ?>" onclick="return deleteNomination( '<?php echo $result['id']; ?>',  '<?php echo $result['nomination_no']; ?>', '<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');"><i class="fa fa-trash-o" aria-hidden="true"></i>	{{ __('messages.Delete') }}</span> 
							@endif
							
							<span onclick="return copyNomination( '<?php echo $result['id']; ?>',  '<?php echo $result['nomination_no']; ?>', '<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');"><i class="fa fa-clone" aria-hidden="true"></i>	{{ __('messages.copnom') }} </span>
					...
					@endif	  
                        ...
                        <div class="nomin-foot">
                          <div class="custom-control custom-checkbox">
						  <!--
							<input type="checkbox"  class="custom-control-input cls_{{$result['id']}}" id="customCheck-04" name="is_scrutiny_completed" value="{{$result['id']}}" onclick="return setRadio('<?php echo $result['id']; ?>', '<?php echo $result['st_code']; ?>', '<?php echo $result['pc_no']; ?>');"> -->
				...
				<div class="card-footer">				  
					<div class="panel-foot">
					 ...
					 $datalink = app(App\Http\Controllers\Nomination\NominationController::class)->getDataLink($results[0]['id']);  ?>
						<?php $stst = app(App\Http\Controllers\Nomination\NominationController::class)->getSchStatus($results[0]['st_code'], $results[0]['pc_no'] ); ?>
						...
						@if($stst==0)
								@if($n!=1)
									...
									@if($isdr > 0) 
											<!--DD<div class="apt-btn text-right" onclick="return openPopup('<?php echo $results[0]['id']; ?>', '<?php echo $results[0]['st_code']; ?>', '<?php echo $results[0]['pc_no']; ?>');"> <a  class="btn btn-lg font-big dark-purple-btn" style="color:white;">{{ __('nomination.PROCCED_TO_SCHEDULE_APPOINTMENT') }} </a> </div>-->
										
											<div class="apt-btn text-right" onclick="return openPopup_prev('<?php echo $results[0]['id']; ?>', '<?php echo $results[0]['st_code']; ?>', '<?php echo $results[0]['pc_no']; ?>');"> <a  class="btn btn-lg font-big dark-purple-btn" style="color:white;">{{ __('nomination.PROCCED_TO_SCHEDULE_APPOINTMENT') }} </a> </div>
									
									@else
										
									
									<!--DD<div class="apt-btn text-right" onclick="return bookAnAppointment('<?php echo $results[0]['id']; ?>', '<?php echo $results[0]['st_code']; ?>', '<?php echo $results[0]['pc_no']; ?>');"> <a  class="btn btn-lg font-big dark-purple-btn" style="color:white;">{{ __('nomination.PROCCED_TO_SCHEDULE_APPOINTMENT') }} </a> </div> -->
											@if($iddataa==0)		
												<div class="apt-btn text-right"  onclick="return bookAnAppointment_prev('<?php echo $results[0]['id']; ?>', '<?php echo $results[0]['st_code']; ?>', '<?php echo $results[0]['pc_no']; ?>');"> <a  class="btn btn-lg font-big dark-purple-btn" style="color:white;">{{ __('nomination.PROCCED_TO_SCHEDULE_APPOINTMENT') }}</a> </div>
											@endif
						...
						@else	
							...
							<!--DD <div class="apt-btn text-right"> <a  class="btn btn-lg font-big dark-purple-btn" style="color:white;" href="{!! url('nomination/book-details') !!}?query={{$link}}&id=<?php echo $datalink; ?>&data={{$link}}">{{ __('nomination.Appointment_Scheduled') }}</a> </div> -->
								@if($iddataa==0)		
									<div class="apt-btn text-right" onclick="return bookAnAppointment_prev('<?php echo $results[0]['id']; ?>', '<?php echo $results[0]['st_code']; ?>', '<?php echo $results[0]['pc_no']; ?>');"> <a  class="btn btn-lg font-big dark-purple-btn" style="color:white;">{{ __('nomination.Appointment_Scheduled') }}</a> </div>
								@endif	
                ...
                <div class="tab-body">
                  <table class="table tableCustom">
					...
					<!-- Pre Scrutiny Submitted from Preview Page -->	
					 ...
					 <?php   
						     $isDraData = app(App\Http\Controllers\Nomination\NominationController::class)->getdateNom($result['st_code'], $result['pc_no']);
					...
					?>
                      ...
                      <tr id="main_{{$result['id']}}">
                        ...
                        <td>
							<div class="">{{ __('nomination.ac') }}  &amp; {{ __('nomination.Name') }}  <span><strong>{{$result['pc_name']}}</strong></span></div>
                             <div>&nbsp;</div>
						 </td>
                        <td>
...
<script>
	...
	function changeDish(ent){
		if(ent!=''){
		   var dd = ent.split('***+++');	
		   window.location.href="<?php echo url('/'); ?>/nomination/nominations?pcs="+dd[0]+'&std='+dd[1];
		}		
	}
