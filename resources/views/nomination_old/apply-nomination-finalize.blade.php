	...
	<div class="container">
	 <div class="step-wrap mt-4" id="ttttt">
		<ul>
		   ...
		   <li class="step-success"><b>&#10004;</b><span>{{ __('step1.step6') }}</span></li>
		   <li class="step-current"><b>&#10004;</b><span>{{ __('step1.step7') }}</span></li>
		</ul>
	 </div>
	</div>
     <section>
      <div class="container p-0">
         <div class="row">
			...
			<input type="hidden" name="nomination_id" value="{{$nomination_id}}">		  
			 <div class="container-fluid">
                ...
                @endif
				  ...
				  <table class="customTable">
					<tbody>
					  ...
					  <tr>
						<td class="td-center"><h5 style="color: #ee577e; font-size: 28px; padding: 12px;">{{ __('finalize.Preview') }}</h5></td>
					  </tr> 
					  ...
					  <tr>
						...
						<td class="td-center"><i>{{ __('step3.nommessage') }} </i></td>
					  </tr> 
					  <tr>
			  ...
			  <tr>
				...
				<td class="td-center"><b>{{ __('finalize.PART1') }}</b></td>
			  </tr> 
			  <tr>
				<td class="td-center">({{ __('finalize.recognized_party') }}) </td> 
			  </tr> 
			  <tr>
			  	<td>{{ __('finalize.nominate_ac') }}<span><b>&nbsp; {{$pc_no}}-  <?php echo app(App\Http\Controllers\Nomination\NominationController::class)->getPcName($st_code, $pc_no); ?></b></span>{{ __('finalize.Assembly_Constituency') }}. </td>
			  </tr>	
			 <tr>
			 	<td class="param-area">
			...
			{{ __('finalize.amb_comp_with') }}  <span>&nbsp; <b>{{$candidate_ac}}-<?php echo app(App\Http\Controllers\Nomination\NominationController::class)->getAcName($candidate_state, $candidate_ac); ?></b></span>{{ __('finalize.amb_comp_with') }} <span><b>&nbsp; {{$candidate_pc}}-  <?php echo app(App\Http\Controllers\Nomination\NominationController::class)->getPcName($candidate_state, $candidate_pc); ?></span></b>{{ __('finalize.Assembly_Constituency') }}. 
				    </p>
					<br>
					<p> {{ __('finalize.My_name_is') }} <span>&nbsp;<b>{{$proposer_name}}</b> </span> {{ __('finalize.and_it_is_entered_at_Sl') }} <span>&nbsp; <b>{{$proposer_serial_no}}</b> </span> {{ __('finalize.in_Part_No') }} <span>&nbsp; <b>{{$proposer_part_no}}</b> </span> {{ __('finalize.of_the_electoral_roll_for') }} <span>&nbsp; <b>{{$proposer_assembly}}-<?php echo app(App\Http\Controllers\Nomination\NominationController::class)->getAcName($st_code, $proposer_assembly); ?>	</b> </span> {{ __('finalize.amb_comp_with') }} <span><b>&nbsp; {{$pc_no}}-  <?php echo app(App\Http\Controllers\Nomination\NominationController::class)->getPcName($st_code, $pc_no); ?></span></b>    {{ __('finalize.Assembly_Constituency') }}. </p>
				 </td>
			 </tr>
			 <tr>
			 	<td>
			...
			@if($recognized_party != '3')	 
			 <tr>
			  ...
			  <tr>
			  	<td class="param-area">
				  <p>
					<strike>{{ __('finalize.nominate_ac') }} <span>&nbsp; <b></b></span>{{ __('finalize.Assembly_Constituency') }}. </strike>
				  </p>  
				  <p><strike> {{ __('finalize.Candidate_name') }}<span>&nbsp; <b></b></span>{{ __('finalize.Father_husband_mother') }}<span>&nbsp; <b></b></span>{{ __('finalize.His_postal_address') }}<span style="width:auto;">&nbsp;  <b></b> </span></strike>
                  
				<strike> 	{{ __('finalize.His_name_is_entered_at_Sl') }} <span>&nbsp; <b></b></span> {{ __('finalize.in_Part_No') }} <span>&nbsp; <b></b></span><span>&nbsp; <b></b></span>  {{ __('finalize.amb_comp_with') }} <span><b>&nbsp; </span></b> {{ __('finalize.Assembly_Constituency') }}. </strike>
				...
				<tr>
					<td>
					  <p><strike>
						{{ __('finalize.We_declare_that_we_are_electors') }}:-</strike>
					  </p>
					</td>
				</tr>
				<tr> 
					  ...
					  <table style="width:100%; text-align: center;" border="1">
						  <tr>
							  ...
							  <table style="width:100%;" border="0">
									<tr>
										<th colspan="2" class="td-center" style="border-bottom: 1px solid #313131;"><strike>{{ __('finalize.Elector_Roll_No') }}</strike></th>
									</tr>
									<tr>
										<th  style=" width: 50%; border-right: 1px solid #313131;"><strike>{{ __('finalize.Part_No_of_Electoral') }}</strike></th>
										<th><strike>{{ __('finalize.SNo_in_that_part') }}</strike></th>
									</tr>
							  </table>  
							</th>
							 		
						  	<th><strike>{{ __('finalize.Full_Name') }}</strike></th>
						  	<th><strike>{{ __('finalize.Signature') }}</strike></th>
						  	<th><strike>{{ __('finalize.Date') }}</strike></th>
						  </tr>
			...
			@endif	
			 ...
			 <tr>
			 	<td class="param-area">
					...
					<span style="width:auto;">&nbsp;  <b></b> </span> {{ __('finalize.His_name_is_entered_at_Sl') }} <span>&nbsp; <b> </b></span> {{ __('finalize.in_Part_No') }} <span>&nbsp;<b> </b></span> {{ __('finalize.of_the_electoral_roll_for') }} <span>&nbsp; </b></span> {{ __('finalize.amb_comp_with') }} <span></span> {{ __('finalize.Assembly_Constituency') }}. </hr>
				    </p>
					<br>
			...
			<tr>
			  ...
			  <tr>
			  	<td class="param-area">
				  <p>
					{{ __('finalize.nominate_ac') }} <span><b>&nbsp; {{$pc_no}}-  <?php echo app(App\Http\Controllers\Nomination\NominationController::class)->getPcName($st_code, $pc_no); ?></span></b>{{ __('finalize.Assembly_Constituency') }}. 
				  </p>  
					 ...
					 <p><b>(c)</b> (i) {{ __('finalize.I_am_set_up') }} <span style="width: auto;"><b> &nbsp; {{$party_id}} </b> </span>  {{ __('finalize.party_which_is_recognized') }} 
					  ...
					  <!-- Stike Start -->
						@if($recognized_party!=3)  		
						 ...
						 <p><strike><b>(c)</b> (ii) {{ __('nomination.i_am_set_1') }}   <span ><b></b> </span>  {{ __('nomination.i_am_set_3') }}   {{ __('nomination.i_am_set_333') }} </strike></p>	
						<p>
		       ...
		       <tr>
			 	<td>
				  <table style="width: 100%; margin: 1.5rem 0;">
					 <tbody>
					   <tr>  
					   	<td>{{ __('finalize.Date') }} <span>&nbsp;<b>{{date("d-m-Y", strtotime($part3_date))}}</b></span></td>
					   	<td class="td-right">
					 ...
					 <tbody>
					   <tr>
					   	<td>
							...
							<div>{{ __('part3a.Date') }}: <b>{{date("d/m/Y", strtotime($date_of_disloyal))}}</b></div>
						</td>
					   	<td class="td-right">
							<div>{{ __('finalize.Signature_of_Candidate') }}</div>
						</td>
	  ...
	  <div class="form-group">
		<div class="col">
		  <a href="{{$href_back}}" id="" class="btn btn-secondary float-left font-big">{{ __('step1.Back') }}</a>
		</div>
		<div class="col ">
		  <div class="form-group row float-right">
		  <div style="background:#ee577f;margin-right:50px;color:white;" class="btn btn-primary save_next font-big" ><a style="background:#ee577f; color:white;" href="<?php echo url('/'); ?>/dashboard-nomination-new">{{ __('step1.Cancel') }}</a></div>  
		  @if($finalize!='yes')
		  <div style="background:#D04A8A;margin-right:50px;" class="btn btn-primary save_next font-big" onclick="return finalize();">{{ __('messages.proSec') }}
		  @else
		  <div style="background:#D04A8A;margin-right:50px;" class="btn btn-primary save_next font-big" onclick="return finalize();">{{ __('messages.proSec') }}
		  @endif	
		  </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom">
      <div class="modal-content">
		...
		<div class="header-caption px-4">
		   ...
		   <p class="font-big">{{ __('messages.However') }}
		    </p> 
			<!--<p>{{ __('messages.printout') }}</p>-->
		</div>		
        </div>
        <!-- Modal footer -->
        <div class="confirm-footer">
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom">
      <div class="modal-content">
        ...
        <div class="pop-header py-4">
          ...
          <h2 class="modal-title">{{ __('finalize.Confirmation') }}</h2> 
		<div class="header-caption px-4">
		  <p class="font-big">{{ __('finalize.finalize_nomination') }} </p>	
		</div>		
        </div>
        <!-- Modal footer -->
        <div class="confirm-footer">
		  <button type="button" class="btn dark-pink-btn font-big mr-4" data-dismiss="modal">{{ __('step1.Cancel') }}</button>
          <button type="button" class="btn dark-purple-btn font-big" onclick="formFinalize('<?php echo $reference_id; ?>');">{{ __('finalize.Ok') }}</button>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom">
      <div class="modal-content">
        ...
        <div class="pop-header py-4">
          ...
          <h2 class="modal-title">{{ __('finalize.Confirmation') }}</h2> 
		<div class="header-caption px-4">
		  <p class="font-big">{{ __('finalize.s') }}</p>	
		</div>		
        </div>
        <!-- Modal footer -->
        <div class="confirm-footer">
		  <button type="button" class="btn dark-pink-btn font-big mr-4" data-dismiss="modal" onclick="return goback();">{{ __('finalize.Ok') }}</button>
        </div>
      </div>
    </div>
  </div><!-- End Of confirm Modal popup Div -->
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom">
      <div class="modal-content">
        ...
        <div class="pop-header py-4">
          ...
          <h2 class="modal-title">{{ __('finalize.Confirmation') }}</h2> 
		<div class="header-caption px-4">
		  <p class="font-big">{{ __('finalize.f') }}</p>	
		</div>		
        </div>
        <!-- Modal footer -->
        <div class="confirm-footer">
		  <button type="button" class="btn dark-pink-btn font-big mr-4" data-dismiss="modal">{{ __('finalize.Ok') }}</button>
        </div>
      </div>
    </div>
  </div><!-- End Of confirm Modal popup Div -->
