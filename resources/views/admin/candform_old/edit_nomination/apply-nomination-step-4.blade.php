					...
					<div class="fullwidth" style="float: left;width: 100%;">
						...
						<div class="col-md-5 float-right">
							<ul class="list-inline float-right">
								<li class="list-inline-item text-right">{{ __('election_details.ref') }}: <b
										style="text-decoration: underline;">{{$reference_id}}</b></li>
								<li class="list-inline-item text-right"><a href="{!! $href_download_application !!}"
										class="btn btn-primary" target="_blank">{{ __('election_details.down') }}</a>
								</li>
							</ul>
						</div>
						@endif
						...
						<fieldset class="py-4 px-5 mt-2 mb-4">
							...
							<div class="row mt-2 mb-4 align-items-center">
								...
								<div class="col-sm-3 col-12 pl-0">
									<input type="number" name="age" placeholder="Enter Age Of Years" min="25"
										class="form-control" value="{{$age}}">
								</div>
							</div>
							<div class="row align-items-center">
								@if($recognized_party==0 or $recognized_party=='1' or $recognized_party=='' or
								$recognized_party==3)
								<?php 
			$dis1='';
			$rec='';
			...
			?>
								...
								<div class="col-sm-12 col-12">
									<div class="d-flex align-items-center my-3">
										...
										<div class="custom-control custom-radio customRadioBtn mr-3"
											onclick="return national();">
											<input type="radio" class="custom-control-input" id="nParty"
												name="recognized_type" value="1" @if($recognized_type==1 or
												$recognized_type=='' ) {{'checked'}}@endif>
											<label class="custom-control-label"
												for="nParty">{{ __('part3.nat') }}</label>
										</div>
										<div class="custom-control custom-radio customRadioBtn mr-3"
											onclick="return state();">
											<input type="radio" class="custom-control-input" id="nonRecg"
												name="recognized_type" value="2" @if($recognized_type==2)
												{{'checked'}}@endif>
											<label class="custom-control-label"
												for="nonRecg">{{ __('part3.stp') }}</label>
										</div>
									</div>
								</div>
								...
								<div class="col-sm-12">
									<div class="d-flex my-3">
										...
										<div class="mt-4" style="width:70%;">
											<select name="party_id" class="form-control" id="national"
												style="display:{{$rec}}" onchange="getPartyVal(this.value);" {{$d1}}>
												<option value="">-- {{ __('part3.ps') }} --</option>
												@foreach($parties as $iterate_party)
												@if($party_id == $iterate_party['party_id'])
												<option value="{{ $iterate_party['party_id'] }}" selected="selected">
													{{ $iterate_party['name'] }}</option>
												@else
												<option value="{{ $iterate_party['party_id'] }}">
													{{ $iterate_party['name'] }}</option>
												@endif
												@endforeach
											</select>

											<select name="party_id" class="form-control" id="state"
												style="display:{{$rec2}}" onchange="getPartyVal(this.value);" {{$d2}}>
												<option value="">-- {{ __('part3.ps') }} --</option>
												@foreach($parties_state as $iterate_party_state)
												@if($party_id == $iterate_party_state['party_id_state'])
												<option value="{{ $iterate_party_state['party_id_state'] }}"
													selected="selected">
													{{ $iterate_party_state['name_party_id_state'] }}</option>
												@else
												<option value="{{ $iterate_party_state['party_id_state'] }}">
													{{ $iterate_party_state['name_party_id_state'] }}</option>
												@endif
											...
											<div><small class="text-black-50">{{ __('part3.symbol') }}</small></div>
										</div>
										<div class="animate-wrap party_wrap" style="width:30%"
											style="display:<?php echo (!empty($setup_partyDatat['party_id']))?'none':'block';?>">
											<div class="animate-help-text dir-lft">
												<div class="help-text">{{ __('messages.arparty') }}</div>
												<div class="animate-icon">
													<div class="box bounce-2"><i class="fa fa-hand-o-left"
								...
								<div class="col-sm-12 col-12">
									<div class="d-flex align-items-center my-3">
										<div>
											<hr
												style="width: 64%; height: -31px; display: block; border-top:2px solid #000; position: absolute; margin-top: 10px;">
											c(ii)&nbsp;</hr>
										</div>
										<div class="custom-control custom-radio customRadioBtn mr-3">
											<input type="radio" class="custom-control-input pParty">
											<label class="custom-control-label"
												for="first">{{ __('part3.recp') }}</label>
										</div>
										<div class="custom-control custom-radio customRadioBtn mr-3">
											<input type="radio" class="custom-control-input">
											<label class="custom-control-label"
												for="second">{{ __('part3.ind') }}</label>
										</div>
										<div class="mr-3">
										</div>
									</div>
			...
			?>
								...
								<div class="col-sm-12 col-12">
									<div class="d-flex align-items-center my-3">
										<p class="mr-2">
											<hr
												style="width:82%; height: -31px; display: block; border-top:2px solid #000; position: absolute; margin-top: 0px;">
											C(i)&nbsp; {{ __('part3.rec') }} </hr>
										</p>
										<div class="custom-control custom-radio customRadioBtn mr-3">
											<input type="radio" class="custom-control-input">
											<label class="custom-control-label"
												for="nonRecg">{{ __('part3.nat') }}</label>
										</div>
										<div class="custom-control custom-radio customRadioBtn mr-3">
											<input type="radio" class="custom-control-input">
											<label class="custom-control-label"
								...
								<div class="col-sm-12 col-12">
									<div class="d-flex align-items-center my-3">
										<div>c(ii)&nbsp;</div>
										<div class="custom-control custom-radio customRadioBtn mr-3">
											<input type="radio" class="custom-control-input pParty" id="first"
												name="unrecognized_type" value="1" @if($unrecognized_type==1 or
												$unrecognized_type=='' ) {{'checked'}} @endif>
											<label class="custom-control-label" for="first"
												onclick="return shows('1');">{{ __('part3.recp') }}</label>
										</div>
										<div class="custom-control custom-radio customRadioBtn mr-3">
											<input type="radio" class="custom-control-input" id="second"
												name="unrecognized_type" value="2" @if($unrecognized_type==2)
												{{'checked'}} @endif>
											<label class="custom-control-label" for="second"
												onclick="return shows('2');">{{ __('part3.ind') }}</label>
										</div>
										<div class="mr-3">

											<div id="setupDiv" style="display:<?php echo $dis1; ?>">
												<select name="party_id2" class="form-control js-example-basic-single"
													title="{{ __('part3.ps') }}" id="setup"
													style="display:<?php echo $dis1; ?>">
													<option value=""></option>
													@foreach($setup_party as $setup_partyDatat)
													@if($party_id2 == $setup_partyDatat['party_id'] and
													$setup_partyDatat['party_id']!=743)
													<option value="{{ $setup_partyDatat['party_id'] }}"
														selected="selected">{{ $setup_partyDatat['name'] }}</option>
													@else selected
													<option value="{{ $setup_partyDatat['party_id'] }}">
														{{ $setup_partyDatat['name'] }} </option>
													@endif
													@endforeach
												</select>
											</div>

											<select name="party_id2" class="form-control" id="setup_independent"
												style="display:<?php echo $dis2; ?>;" disabled>
												@foreach($setup_party as $setup_partyDatat)
												@if($setup_partyDatat['PARTYTYPE']=='Z')
												<option value="{{ $setup_partyDatat['party_id'] }}" selected="">
													{{ $setup_partyDatat['name'] }}</option>
												@endif
												@endforeach
											</select>
								...
								<div class="col-12">
									<div class="form-group mt-2 mb-2">
										...
										<div class="d-flex">
											<div class="col-sm-4 col-12">1.<input list="sym1" type="text"
													name="suggest_symbol_1" id="suggest_symbol_1" class="form-control"
													value="{{$suggest_symbol_1}}" onmouseover="return getSymbol(1);"
													onkeypress="return getSymbol(1);">
												<datalist id="sym1">
													<option value="Edge">
												</datalist>
											</div>
											<div class="col-sm-4 col-12">2. <input list="sym2" type="text"
													name="suggest_symbol_2" id="suggest_symbol_2" class="form-control"
													value="{{$suggest_symbol_2}}" onmouseover="return getSymbol(2);"
													onkeypress="return getSymbol(2);">
												<datalist id="sym2">
													<option value="Edge">
												</datalist>
											</div>
											<div class="col-sm-4 col-12">3. <input list="sym3" type="text"
													name="suggest_symbol_3" id="suggest_symbol_3" class="form-control"
													value="{{$suggest_symbol_3}}" onmouseover="return getSymbol(3);"
													onkeypress="return getSymbol(3);">
												<datalist id="sym3">
													<option value="Edge">
												</datalist>
											</div>
								...
								<div class="col-sm-12 col-12">
									<div class="d-flex align-items-center my-3">
										...
										<div style="width: 12.50%;">
											<input type="text" list="language" id="langData" name="language"
												class="form-control alphaonly" value="{{$language}}"
												onmouseover="return getLangauge();" style="width: 120%;"
												autocomplete="off">
										</div>
								...
								<div class="col-sm-12 col-12">
									<div class="one-param">
											...
											<div class="info-checkbox mb-3">
												<div class="custom-control custom-checkbox customCheckBtn">
													<input type="checkbox" class="custom-control-input"
														name="not_applicable" id="customCheck013" checked disabled
														@if($not_applicable=='on' ){{'checked'}}@endif>
													<label class="custom-control-label" for="customCheck013"
														style="background: none; position: static; margin-top: -15px; margin-left: 18px;">{{ __('part3.np') }},</label>
												</div>
										...
										<span>
											*{{ __('part3.further') }}
											<select name="category" class="form-control" disabled>
												...
												<option value="{{$iterate_category['id']}}" selected="selected">
													{{$iterate_category['name']}}</option>
												@else
												<option value="{{$iterate_category['id']}}">
													{{$iterate_category['name']}}</option>
												@endif
												@endforeach
											</select>
											<hr
												style="width:65%; height: -31px; display: block; border-top:2px solid #000; position: absolute; margin-top: 15px;">
											{{ __('part3.caste') }} </hr>
											<select class="form-control" disabled>
												<option value="">-- {{ __('part3.Select') }} --</option>

											</select>
											{{ __('part3.rel') }} <input type="text" class="form-control"
												readonly>{{ __('part3.area') }}.
										</span>
										<!--End Strike Content -->
										...
										<span id="TTT" style="display:<?php echo $chk; ?>;">
											*{{ __('part3.further') }}
											...
											<select name="part3_cast_state" class="form-control" id="part3_cast_state">
												...
												<option value="{{ $iterate_state['st_code'] }}" selected="selected">
													{{ $iterate_state['st_name'] }}</option>
												@else
												<option value="{{ $iterate_state['st_code'] }}">
													{{ $iterate_state['st_name'] }}</option>
												@endif
												@endforeach
											</select>
											{{ __('part3.rel') }} <input type="text" name="part3_address"
												id="part3_address" class="form-control"
												value="{{$part3_address}}">{{ __('part3.area') }}.
										</span>
										@endif
										@endif
										<br />
										...
										{{ __('part3.also') }}
										<select name="part3_legislative_state" class="form-control"
											id="part3_legislative_state" disabled>
											<option value="">-- {{ __('part3.Select') }} --</option>
											@foreach($states as $iterate_state)
										...
										<input type="hidden" name="part3_legislative_state" value="{{$st_code}}"
											id="part3_legislative_state">
						...
						<div class="row my-3">
							<div class="col-sm-6 col-12"><strong>{{ __('part3.date') }}:</strong> <span>
									<input type="hidden" name="part3_date" id="part3_date" value="{{$part3_date}}"
										readonly="readonly">
									<input type="text" name="part3_date" id="part3_date" value="{{$part3_date}}"
						...
						<div class="card-footer">
							<div class="row align-items-center">
								<div class="col-sm-6 col-12"> <a href="{{$href_back}}" id=""
										class="btn btn-lg btn-secondary font-big">{{ __('step1.Back') }}</a> </div>
								<div class="col-sm-6 col-12">
									<div class="apt-btn text-right">

										<a href="<?php echo url('/'); ?>/ropc/dashboard"
											class="btn btn-lg font-big dark-pink-btn">{{ __('step1.Cancel') }}</a>
										&nbsp;

										<?php if($recognized_party=='2'){ ?>
										<button type="submit" class="btn btn-lg font-big dark-purple-btn pop-actn"
											onclick="return chkCat();">{{ __('step1.Save_Next') }}</button>
										<?php } else { ?>
										<button type="submit" class="btn btn-lg font-big dark-purple-btn pop-actn"
											onclick="return chkCat();">{{ __('step1.Save_Next') }}</button>
										<?php }  ?>
