	...
	<table class="table-strip" style="width: 100%;" border="1" align="center">
		...
		<tbody>
			...
			<?php
				...
				$pc= getpcbypcno($each_data['st_code'],$each_data['pc_no']);
			?>
			<tr>
                <td>{{ $srno++ }}</td>
				...
				<td>{{ !empty($each_data['DIST_NO_HDQTR']) ? $each_data['DIST_NO_HDQTR'].'-'.$each_data['DIST_NAME'] : '' }}</td>
				<td>{{ !empty($each_data['pc_no']) ? $each_data['pc_no'].'-'.getpcbypcno($each_data['st_code'], $each_data['pc_no'])->PC_NAME : '' }}</td>
                <td>
					<?php 
