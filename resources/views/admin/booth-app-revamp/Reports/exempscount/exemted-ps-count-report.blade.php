          ...
          <table class="table table-bordered " id="list-table" data-page-length='50'>
           <thead>
            <tr> 
              ...
              <th>State/UT Name</th>
              <th>Total PS</th>
              <th>Total Exempted PS</th>
            </tr>
          </thead>
          <tbody>
			@php
				$total_ps = $total_exemps = 0;$i=1;
			 @endphp
            ...
            <td>
              @if($result['total_exemps'] > 0)<a href="<?php echo $result['href'] ?>"><span>{!! $result['total_exemps'] !!} </span></a>@else {!!$result['total_exemps']!!} @endif
            </td>
            </tr>
			@php
			$i++;
				$total_ps += $result['total_ps'];
				$total_exemps +=  $result['total_exemps'];
			 @endphp
