              ...
              <tbody>
                ...
                <tr class="">
                  ...
                  <td class="level1"><p>Name: <b>{{$result['name']}}</b></p><p>Mobile: <b>{{$result['mobile']}}</b> </p><p>Status: <b>{{$result['is_active']}}</b> </p>
                    ...
                    <input type="hidden" value="{{$result['sector_id']}}" class="ps_no" name="ps_no">
      ...
      <form class="form-horizontal" method="post" action="{!! $action !!}">
        ...
        <div class="modal-body">
          ...
          <div class="form-group mb-1">
            <label class="col-md-3  pull-left">Sector</label>
            <div class="pull-left col-md-9" id="ps_no_div">
              <div class="row">
                <select class="sumoselect form-control ps_no sector_no" name="sector_no" id="ps_no">
                  @foreach($sector_info as $iterate_sm)
                    <option value="{{$iterate_sm['sector_id']}}">{{$iterate_sm['sector_id']}}-{{$iterate_sm['sector_name']}}</option>
                  @endforeach
                </select>
              </div>
            </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('.edit_id_button').click(function(e){
	  ...
	  $('#add_new_so_modal .sector_id').val($(this).parent('td').find('.sector_id').val());
    ...
    $('#add_new_so_submit').click(function(){
      $.ajax({
        url: "{!! url('/roac/booth-app-revamp/assign-so/save-via-ajax-new') !!}",
        type: 'POST',
        ...
        success: function(json) {
          ...
          if(json['success'] == false){
			...
			if(json['errors']['sector_no']){
              $("#add_new_so_modal .sector_no").addClass("input-error");
              $("#add_new_so_modal .sector_no").after("<span class='text-error text-danger text-right pull-right'>"+json['errors']['sector_no'][0]+"</span>");
            }
            ...
            if(json['errors']['sector_no']){
              ...
              $("#add_new_so_modal #ps_no_div").after("<span class='text-error text-danger text-right pull-right'>"+json['errors']['sector_no'][0]+"</span>");
            }
    ...
    $('#add_sub_so_submit').click(function(){
      $.ajax({
        url: "{!! url('/roac/booth-app-revamp/assign-so/save-sub-so-new') !!}",
        type: 'POST',
