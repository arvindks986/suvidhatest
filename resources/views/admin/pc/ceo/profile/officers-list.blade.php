          ...
          @if(!empty($results))
           ...
           @foreach($results as $result)
            <tr id="row_{{$i}}">
              ...
              <td>{{$result['email']}}</td>
              <td>{{$result['mobile']}}</td>
              <td><input type="hidden" name="reset-path" class="reset-path" value="{!! $result['hash_id'] !!}"><button class="btn btn-primary reset_pin_button">Reset Pin</button></td>
            </tr>
            <?php $i++; ?>
            @endforeach
...
$(document).ready(function(e){
  ...
  $('#ceo_pin_submit').click(function(e){
      $.ajax({
        url: "{!! url('/pcceo/officer/update-password') !!}",
        type: 'POST',
