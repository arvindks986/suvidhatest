...
<main role="main" class="inner cover mb-1">     
    <section class="breadcrumb-section">
        <div class="container-fluid">
            <div class=" row">
                <div class="col"><h2 class="mr-auto">Fund From Party: {{$count}}</h2></div> 
...
<tr>
<td>@if(!empty($candDetails->constituency_no)) {{ $pc->PC_NO}} - {{ $pc->PC_NAME}} @endif</td>
...
<td>@if(!empty($totalpartyfund)) {{$totalpartyfund}} @else 0 @endif</td>
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        $.ajax({
            type: "get",
            url: "{{url('/')}}/pcceo/candidateListBypartyfundgraph/{{$cons_no}}",
            dataType: "json",
            success: function (response) {
                var data = google.visualization.arrayToDataTable(response);
                var options = {
                    chart: {
                        title: 'Overall summary of Taken funds from party',                         
                    },
