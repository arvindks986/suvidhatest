...
$last_date_prescribed_acct_lodge = !empty($noticeatCEO[0]->last_date_prescribed_acct_lodge) && strtotime($noticeatCEO[0]->last_date_prescribed_acct_lodge) > 0 ?date('d-m-Y', strtotime($noticeatCEO[0]->last_date_prescribed_acct_lodge)) : "22-06-2019";
@endphp
<main role="main" class="inner cover mb-1">     
    <section class="breadcrumb-section">
        <div class="container-fluid">
            <div class=" row">
                <div class="col-md-5 mt-2 mb-2"><h5 class="mr-auto">Notice AT CEO: {{$count}}</h5></div> 
                <div class="col-md-7 mt-2 mb-2 text-right"><p class="mb-0">
                        ...
                        <a href="{{url('/pcceo/noticeatceoEXL')}}/{{base64_encode($cons_no)}}" class="btn btn-info" role="button">Export Excel</a> &nbsp;&nbsp;

                        <b></b><!--<a href="{{url('/pcceo/statusExpdashboard')}}"> <button type="button" id="Back" class="btn btn-primary">Back</button></a>-->
						<a href="##" onClick="history.go(-1); return false;"><button type="button" id="Back" class="btn btn-primary">Back</button></a> 
...
<td>@if(strtotime($candDetails->finalized_date)>0) {{ date('d-m-Y',strtotime($candDetails->finalized_date))}}  @else {{ 'N/A'}} @endif</td>
<td>@if(strtotime($candDetails->date_orginal_acct)>0) {{ date('d-m-Y',strtotime($candDetails->date_orginal_acct))}} @else {{ 'N/A'}} @endif</td>
<td>@if(strtotime($candDetails->date_of_issuance_notice)>0) {{  date('d-m-Y',strtotime($candDetails->date_of_issuance_notice))}} @else {{ 'N/A'}} @endif</td>
...
<td>  @if(!empty($candDetails->pc_no))
                <a href="{{url('/')}}/pcceo/printScrutinyReport/{{base64_encode($candDetails->candidate_id)}}" class="btn btn-primary btn-sm width-75" target="_blank">Scrutiny Report</a> 
                @endif</td>
</tr>
@endforeach 
@endif 
