...
<section>	 
  ...
  <div class="card text-left" style="width:100%; margin:0 auto;">
    ...
    <div class="card-body">  
          ...
          @endif
  <form enctype="multipart/form-data" id="changepassword_form" method="POST"  action="{{url('pcceo/change-password') }}" onsubmit="return checkPassword(this);">
	  {{ csrf_field() }}
  ...
  function checkPassword(str)
  {
   ...
   // var re = /^(?=.*\d)(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,}$/;
    ...
    if(document.getElementById("new-password").value == "") {
      ...
      jQuery('.errormsg').html('');
      jQuery('.newpassword').html('New Password must contain only letters,numbers and special characters(!$#%@) and atleast 8 characters!');
      jQuery( "input[name='new-password']" ).focus();
			return false;
    }
