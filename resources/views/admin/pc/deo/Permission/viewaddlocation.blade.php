...
//                    alert(response);exit;
            ...
            $.ajax({
                url: base_url + '/pcdeo/getallacloc',
                type: 'POST',
                ...
                success: function (data)
                {
                  ...
                  if(data != '')
                  {
                       str +="<table id='list-table' class='table table-striped table-bordered table-hover' style='width:100%'><thead><th>S.no.</th><th>Location Name</th><th>Location Address</th><th>Edit/Update</th></thead><tbody>";
                       for (var i = 0; i < cnt; i++) {
                           var id=data[i]['id'];
                           var lname=data[i]['location_name'];
                           var ladd=data[i]['location_details'];
                           str +="<tr><td>"+id+"</td><td>"+lname+"</td><td>"+ladd+"</td><td id='edit'><a href='{{url('/pcdeo/locationeditpermsn')}}/"+id+"'><span class=btn btn-success float-right>Edit</span></a></td></tr>"
                       }
                       str +="</tbody></table>";
                       $('#viewps').html(str);
