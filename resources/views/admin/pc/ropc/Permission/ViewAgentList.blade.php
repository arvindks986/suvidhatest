...
<main role="main" class="inner cover mb-3 mb-auto">
    <section class="statistics">
        <div class="container-fluid mt-5 mb-5">
		  ...
		  <a type="button" href="{{url('/ropc/permission/viewagent')}}" class="btn btn-sm btn-outline-primary">View</a>
                  <a type="button" href="{{url('/ropc/permission/agentCreation')}}" class="btn btn-sm btn-primary">Add</a>
				  </div></div>
              </div>
            </div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td id="setStatus"><span class="btn btn-secondary setStatus" id="{{$data->is_active}}{{'#'}}{{$data->id}}">{{'Active'}} </span></td>
                 ...
                 @endif
                <td id="edit"><a href="{{url('/ropc/permission/editagent')}}/{{$data->id}}"><span class=" btn btn-success">Edit</span></a></td>
            </tr>
            @endforeach
            @endif
        </tbody>
...
//            alert(getArray[0]);exit;
            ...
            var res=confirm('Are you sure you want to Inctive this user')
                ...
                {
                    $.ajax({
                        url: base_url + '/ropc/permission/agentstatus',
                        type: 'POST',
