...
<div class="sidebar__inner">
    ...
    @endif
			...
			@if(!empty($getAuthorityDetails))
                      ...
                      <form class="form-horizontal" method="POST" action="{{url('/ropc/permission/editauthority')}}">
                          {{csrf_field()}}
                        <div class="form-group row">
                          ...
                          <option value="{{$data->auth_type_id}}" selected>{{$data->auth_type_name}}</option>
                            @if(!empty($authority))
                            @foreach($authority as $data1)
                            @if($data1->id != $data->auth_type_id)
