...
<td>  @if(!empty($candDetails->pc_no))
              <span class="btn-secondary text-white btn btn-sm width-100">Not-Started</span>
                @endif</td>
</tr>
@endforeach 
@endif 
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        $.ajax({
            type: "get",
            url: "{{url('/')}}/eci-expenditure/getpendingcandidateListgraph/{{$st_code}}/{{$cons_no}}",
            dataType: "json",
            success: function (response) {
                var data = google.visualization.arrayToDataTable(response);
                var options = {
                    chart: {
                        title: 'Overall summary of Pending / Not File',                         
                    },
