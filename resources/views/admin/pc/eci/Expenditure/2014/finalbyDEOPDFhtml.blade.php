...
<th>Date of Receipt By CEO</th>
     ...
     $count = 1; 
        $TotalUsers = 0;
        @endphp
         @forelse ($EcifinalbyDEOMISPDF as $candDetails)

         @php
          // dd($candDetails);
         $TotalUsers =count($EcifinalbyDEOMISPDF);
...
<td>@if(!empty($lodgingDate) && $lodgingDate !='0000-00-00' && $lodgingDate > '05-12-2019') {{ date('d-m-Y',strtotime($lodgingDate))}} @else {{ 'N/A'}} @endif</td>
