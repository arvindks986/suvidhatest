...
<main role="main" class="inner cover mb-1">     
    <section class="breadcrumb-section">
        <div class="container-fluid">
            <div class=" row">
                <div class="col-md-5 mt-2 mb-2"><h5 class="mr-auto">Disqualified: {{$count}}</h5></div> 
                <div class="col-md-7 mt-2 mb-2 text-right"><p class="mb-0 text-right">
                       ...
                       <!-- <a href="{{url('/eci-expenditure/pendingatceoPDF')}}/{{base64_encode($st_code)}}/{{base64_encode($cons_no)}}" class="btn btn-info" role="button">PDF Download</a>--> &nbsp;&nbsp;
                        <a href="{{url('/eci-expenditure/disqualifiedbyeciEXL')}}/{{base64_encode($st_code)}}/{{base64_encode($cons_no)}}" class="btn btn-info" role="button">Export Excel</a> &nbsp;&nbsp;
                            ...
                            <table id="example" class="table table-striped table-bordered table-hover" style="width:100%">
                                <thead>
                                    <tr>
										...
										<th>Date of Disqualified</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <?php $j = 0; ?>
                                @if(!empty($getdisqualifiedcandidateListbyECI))
...
<td>@if(!empty($candDetails->disqualified_date)) {{ date('d-m-Y',strtotime($candDetails->disqualified_date))}}  @else {{ 'N/A'}} @endif</td> 
