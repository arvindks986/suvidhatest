		...
		<div class="form-group col-md-6">
         ...
         <div class="form-group col-md-6"> <label>Constituency </label> 
            ...
            @foreach($list_const as $a)
                <option value="{{$a->CONST_NO}}-{{$a->PC_NAME}}" selected="{{$a->CONST_NO}}-{{$a->PC_NAME}}" >{{$a->PC_NAME}}</option>   
            @endforeach
            @endif  
            </select>
          </div>
...
<section>
	<div class="container-fluid">
        ...
        <span id="timeRange">Report of:  {{ date('d-m-Y')}} </span> &nbsp;&nbsp;&nbsp;&nbsp;
                 <span id="caseType">Constituency: {{$a->PC_NAME}}</span>
      </h3>
			</div>
		</div>
	</div>
 ...
 <div class="container">
  ...
  <br>
	...
	<div class="col text-right">
	  <span class="report-btn" id="export-btn"><a class="btn btn-primary" href="{{ url('/ropc/reportspdfview/'.base64_encode('all').'/'.base64_encode('all')) }}" title="Download PDF" target="_blank">Export PDF</a></span>
              <span class="report-btn" id="export-csv-btn"><a class="btn btn-primary" href="{{ url('/ropc/reportexcelview/'.base64_encode('all').'/'.base64_encode('all')) }}" title="Download Excel" target="_blank">Export Excel</a></span>
	  </div>
	  </div>
 ...
 function getDataByTimeInterval(val){   
   ...
   var caseType = jQuery("#constituency option:selected").val();
    ...
    if(from_date!='' && to_date!=''){
        jQuery.ajax({
            type:'POST',
            url:'<?php echo url('/') ?>/ropc/range-datewisereport', //Make sure your URL is correct
            data:{'_token':'<?php echo csrf_token() ?>', 'from_date': from_date,'to_date': to_date,'const': caseType1},
            success:function(data){         
                //alert('FinalResponse->'+data); 
                //alert(getTimeName);
                var exportpdfbtn = '<span class="report-btn" id="export-btn"><a class="btn btn-primary" href="<?php echo url('/') ?>/ropc/reportspdfview/'+btoa(dtStr)+'/'+btoa(caseType1)+'" title="Download to PDF" target="_blank">Export PDF</a></span>';
