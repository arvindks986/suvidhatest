        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12" id="reportdata">
                     <table id="list-table" class='table table-bordered' style='width:100%'><thead><tr><th>Reference No.</th><th>User Name</th><th>User Type</th><th>Permission Type</th><th>Permission Mode</th><th>DateTime of Submission</th><th>Status</th></tr></thead>
					  <tbody>
                    @if(!empty($totalPermissionReport))
                    @foreach($totalPermissionReport as $rdata)
                       
                            <tr><td><a href='{{url('/aro/permission/permissiondetailsview')}}/{{$rdata->permission_id}}/{{$rdata->location_id}}/{{$rdata->approved_status}}' >{{$rdata->permission_id}}</a></td>
...
//        alert(statusid);exit;
        $.ajax({
            url:base_url+'/aro/permission/permissioncountdetails',
            type: 'POST',
...
//                alert(data);
                  ...
                  {
//                      alert(data);
                      str +="<table id='list-table' class='table table-bordered' style='width:100%'><thead><tr><th>Permission ID</th><th>User Name</th><th>User Type</th><th>Permission Type</th><th>Permission Mode</th><th>DateTime of Submission</th><th>Status</th></tr></thead><tbody>";
                      $('#details').css('display','');
                      for (var i = 0; i < cnt; i++) {
                       var id=data[i]['id'];
                       ...
                       var status = "<p class='text-danger'>Cancelled</p>";
						...
						str +="<tr><td><a href='{{url('/aro/permission/permissiondetailsview')}}/"+id+"/"+location_id+"/"+astatus+"' >"+id+"</a></td><td>"+name+"</td><td>"+role_name+"</td><td>"+p_name+"</td><td>"+pstatus+"</td><td>"+added_at+"</td><td>"+status+"</td></tr>";
                  }
                  str +="</tbody></table>";
                  $('#reportdata').html(str);
                  $('#list-table').DataTable();
...
//        var token = $('meta[name="csrf-token"]').attr('content');
//        $.ajax({
//            url:base_url+'/aro/permission/permissiondetailsview',
//            type: 'POST',
