...
<main role="main" class="inner cover mb-1">     
    <section class="breadcrumb-section">
        <div class="container-fluid">
            <div class=" row">
                <div class="col"><h2 class="mr-auto">Loged Account : {{$count}}</h2></div> 
...
<div class="card text-left" style="width:100%;">
	...
	<div class="card-body" id="demo" class="collapse show">  
		<table id="example" class="table table-striped table-bordered table-hover" style="width:100%">
			...
			<?php $j = 0; ?>
			@if(!empty($logedAccount))
			@foreach($logedAccount as $candDetails)  
			<?php
			...
			?>
			<tr>
				<td>@if(!empty($pc->PC_NO)) {{ $pc->PC_NAME}}-{{ $pc->PC_NAME}} @endif</td>
					<td>
...
<script type="text/javascript">
   /* google.charts.load('current', {'packages': ['bar']});
    ...
    function drawChart() {
        ...
        $.ajax({
            type: "get",
            url: "{{url('/')}}/ropc/logedaccountgraph",
            dataType: "json",
            success: function (response) {
                var data = google.visualization.arrayToDataTable(response);
                var options = {
                    chart: {
                        title: 'Overall summary of Loged Account',                         
                    },
