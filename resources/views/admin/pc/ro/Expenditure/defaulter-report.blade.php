		...
		<div class="card-body" id="demo" class="collapse show">  
			<table id="example" class="table table-striped table-bordered table-hover" style="width:100%">
				...
				<tr>
					...
					<td>@if(!empty($candDetails->totalobseramnt)) {{$candDetails->totalobseramnt}} @endif</td>
					<td>@if(!empty($candDetails->totalcandamnt)) {{$candDetails->totalcandamnt}} @endif</td>
...
<script type="text/javascript">
 /*   google.charts.load('current', {'packages': ['bar']});
    ...
    function drawChart() {
        ...
        $.ajax({
            type: "get",
            url: "{{url('/')}}/ropc/getdefaultercandidateListgraph",
            dataType: "json",
            success: function (response) {
                var data = google.visualization.arrayToDataTable(response);
                var options = {
                    chart: {
                        title: 'Overall summary of Defaulter Case',                         
                    },
