...
<main role="main" class="inner cover mb-1">     
    <section class="breadcrumb-section">
        <div class="container-fluid">
            <div class=" row">
                <div class="col"><h2 class="mr-auto">Fund From Other Sources: {{$count}}</h2></div> 
                                ...
                                <tr>
								 <td>@if(!empty($pc->PC_NO))  {{ $pc->PC_NO}}-{{ $pc->PC_NAME}} @endif</td>
                                   <td>
                                    ...
                                    <td>@if(!empty($candDetails->other_source_amount)) {{$candDetails->other_source_amount}} @endif</td>
                                   <td >
                                            
                                    <a href="{{url('/')}}/ropc/printScrutinyReport/{{base64_encode($candDetails->candidate_id)}}" class="btn btn-primary" target="_blank">DEO Scrutiny Report</a> 
                                        </td>
...
<?php //print_r($PreviewData);die;    ?>
            <div class="modal-body">
                <div class="col"><center><h4>Scrutiny Report Filed By DEO</h4></center></div>
                <br>
...
<script type="text/javascript">
 /*   google.charts.load('current', {'packages': ['bar']});
    ...
    function drawChart() {
        ...
        $.ajax({
            type: "get",
            url: "{{url('/')}}/ropc/othersfundgraph",
            dataType: "json",
            success: function (response) {
                var data = google.visualization.arrayToDataTable(response);
                var options = {
                    chart: {
                        title: 'Overall summary of Taken funds from other sources',                         
                    },
