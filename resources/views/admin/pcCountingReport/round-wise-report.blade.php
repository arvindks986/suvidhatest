...
<div class="btns-actn">
{{ Form::model('', ['action' => 'Admin\PcWiseRoundReport\PcWiseRoundReportController@getCompleteResult', 'name'=>'pdfForm' ]) }}
...
<div class="btns-actn">
{{ Form::model('', ['action' => 'Admin\PcWiseRoundReport\PcWiseRoundReportController@csvDownload', 'name'=>'csvDownload' ]) }}
...
<!--<div class="col-md-9"><span> PC : </span><span id="pcname" class="bold"> Delhi </span> </div>-->
</div>
</div>
...
function getResultECI(){ 
	...
	$.ajax({
			type: "POST",
			url: "<?php echo url('/'); ?>/<?php echo $url; ?>/get-all-result-eci-pcwise", 
			data: {
				"_token": "{{ csrf_token() }}",
				"st_code": state,
				"pc": pc,
...
function getPcByStateId(st_code){ 
	...
	$.ajax({
			type: "POST",
			url: "<?php echo url('/'); ?>/<?php echo $url; ?>/get-pc-by-state-id-eci-pcwise", 
			data: {
				"_token": "{{ csrf_token() }}",
				"st_code": st_code
				},
...
function getAcByStateAndPcId(pcId){ 
   ...
   $('#valShow').hide();
	...
	$.ajax({
			type: "POST",
			url: "<?php echo url('/'); ?>/<?php echo $url; ?>/get-ac-by-state-and-pc-id-eci-pcwise", 
			data: {
				"_token": "{{ csrf_token() }}",
				"st_code": state,
				"pcId": pcId
