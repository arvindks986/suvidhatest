...
class ROPCNomModel extends Model
{
	...
	public function candidatelist($st_code, $const_no)
	{
		$list = DB::table('candidate_nomination_detail')
			...
			->select(
				'candidate_personal_detail.cand_name',
				'candidate_personal_detail.cand_mobile',
	...
	{
		$list = DB::table('candidate_nomination_detail')->where('candidate_nomination_detail.st_code', '=', $st_code)
			->where('candidate_nomination_detail.pc_no', '=', $const_no)
			->where('candidate_nomination_detail.application_status', '<>', '11')
	...
	{
		$list = DB::table('candidate_affidavit_detail')->where('st_code', '=', $st_code)
			->where('pc_no', '=', $const_no)->orderBy('added_create_at', 'ASC')->get();

		return $list;
	}
	public function getcounteraffidavite($st_code, $const_no)
	{
		$list = DB::table('candidate_counteraffidavit_detail')->where('st_code', '=', $st_code)
			->where('pc_no', '=', $const_no)->orderBy('added_create_at', 'ASC')->get();
	...
	{
		$list = DB::table('candidate_counteraffidavit_detail')->where('st_code', '=', $st_code)->where('pc_no', '=', $const_no)
			->where('nom_id', '=', $nomid)->orderBy('added_create_at', 'ASC')->get();
	...
	public function getnominationiscriminal($st_code, $const_no)
	{
		...
		$list = DB::table('candidate_nomination_detail')
			...
			->select(
				'candidate_personal_detail.cand_name',
				'candidate_personal_detail.cand_mobile',
				'candidate_personal_detail.is_criminal',
				'candidate_personal_detail.candidate_father_name',
				'candidate_personal_detail.candidate_id',
				'candidate_nomination_detail.nom_id'
			)->get();
