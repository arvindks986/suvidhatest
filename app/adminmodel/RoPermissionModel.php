...
use DB;
class RoPermissionModel extends Model 
{ 
    ...
    public function getallAC($st,$pc)
    {
            $data = DB::table('m_ac')
                    ->select('AC_NAME','AC_NO')
                    ->where('ST_CODE',$st )
                    ->where('PC_NO',$pc)
                    ->get()->toArray();
		return $data;
    ...
    public function getPermissionDetails($stcode,$dist,$pcno,$role)
    {
        //DB::enableQueryLog();
...
//                ->where(array('a.st_code'=>$stcode,'a.pc_no'=>$pcno))
//                ->where(array('a.st_code'=>$stcode,'a.dist_no'=>$pcno))
                ->where(array('a.st_code'=>$stcode,'a.pc_no'=>$pcno))
...
//        DB::enableQueryLog();
    ...
    public function getNodaldetails($id,$ac,$pc,$dist)
    {
//        DB::enableQueryLog();
        if($ac == '0' || $ac == 'NULL')
        {
        $data=DB::table('permission_assigned_auth as a')
         ...
         $data1=DB::table('permission_assigned_auth as a')
                ...
                ->select('a.*','b.*','c.name as auth_name')
                ->where('a.permission_request_id',$id)
                //->where(array('e.pc_no'=>$pc,'e.dist_no'=>$dist))
...
//                if($status == 2 || $status == 3)
//                {
                 $data->select('b.*','a.approved_status','a.cancel_status');
//                }
//                else
//                {
...
//                }
	...
	public function getAllACAuthorityData($stcode,$dist,$pc,$acno)
    {
        $data=DB::table('authority_masters as a')
                ->join('authority_type as b','a.auth_type_id','=','b.id')
                ->select('a.*','b.name as auth_type_name','a.id as nodal_id')
                ->where(array('a.st_code'=>$stcode,'a.ac_no'=>$acno,'a.pc_no'=>$pc))
                ->get()->toArray();
        
        $data1=DB::table('authority_masters as a')
                ->join('authority_masters_mapping as e','e.authority_masters_id','=','a.id')
                ->join('authority_type as b','e.auth_type_id','=','b.id')
                ->select('a.*','b.name as auth_type_name','a.id as nodal_id','e.*')
                ->where(array('a.st_code'=>$stcode,'e.ac_no'=>$acno,'e.pc_no'=>$pc))
                ->where('e.is_active',1)
                ->get()->toArray();
...
//                ->join('user_role as c','c.role_id','=','a.role_id')
    ...
    public function totalPermissionReport($st,$pc)
    {
        //DB::enableQueryLog();
        $data=DB::table('permission_request as a')
...
//        print_r($where);die;
        $data=DB::table('permission_request as a')
                ...
                ->where(array('a.st_code'=>$where[0],'a.pc_no'=>$where[1]))
