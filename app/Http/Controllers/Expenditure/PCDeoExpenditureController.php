...
class PCDeoExpenditureController extends Controller {
    ...
    public function candidateListBydataentryStart(Request $request, $pc) { //dd($request->all());
        ...
        try {
            if (Auth::check()) {
                ...
                return view('admin.pc.deo.Expenditure.dataentrystart-report', ['user_data' => $d, 'DataentryStartCandList' => $DataentryStartCandList, 'edetails' => $ele_details, 'cons_no' => $cons_no, 'count' => count($DataentryStartCandList)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function candidateListByfinalizeData(Request $request, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                //dd($DataentryStartCandList);
                return view('admin.pc.deo.Expenditure.finalize-report', ['user_data' => $d, 'finalCandList' => $finalCandList, 'edetails' => $ele_details, 'cons_no' => $cons_no, "count" => count($finalCandList)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
        ...
        try {
            if (Auth::check()) {
                ...
                if (empty($logedAccount) || $logedAccount == '')
                    $logedAccount = array();
                //dd($logedAccount);
                return view('admin.pc.deo.Expenditure.logedaccount-report', ['user_data' => $d, 'logedAccount' => $logedAccount, 'edetails' => $ele_details, 'cons_no' => $cons_no, 'count' => count($logedAccount)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function candidateListBynotintime(Request $request, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                //dd($DataentryStartCandList);
                return view('admin.pc.deo.Expenditure.notintime-report', ['user_data' => $d, 'notinTime' => $notinTime, 'edetails' => $ele_details, 'cons_no' => $cons_no, "count" => count($notinTime)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function candidateListByformatedefects(Request $request, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                //dd($DataentryStartCandList);
                return view('admin.pc.deo.Expenditure.formatedefects-report', ['user_data' => $d, 'formateDefects' => $formateDefects, 'edetails' => $ele_details, 'cons_no' => $cons_no, 'count' => count($formateDefects)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
...
$st_code=!empty($st_code) ? $st_code : 0;
    ...
    //dd($DataentryStartCandList);
    return view('admin.pc.deo.Expenditure.expenseunderstated-report', ['user_data' => $d, 'expenseunderstated' => $expenseunderstated, 'edetails' => $ele_details, 'cons_no' => $cons_no,
...
$st_code=!empty($st_code) ? $st_code : 0;
    ...
    // dd($partyfund);
    return view('admin.pc.deo.Expenditure.partyfund-report', ['user_data' => $d, 'partyfund' => $partyfund, 'edetails' => $ele_details, 'cons_no' => $cons_no, "count" => count($partyfund)]);
} else {
    return redirect('/officer-login');
}
} catch (Exception $ex) {
...
$st_code=!empty($st_code) ? $st_code : 0;
        ...
        //dd($DataentryStartCandList);
        return view('admin.pc.deo.Expenditure.otherfund-report', ['user_data' => $d, 'otherfund' => $otherfund, 'edetails' => $ele_details, 'cons_no' => $cons_no, "count" => count($otherfund)]);
    } else {
        return redirect('/officer-login');
    }
} catch (Exception $ex) {
...
// end candidateListByexeedceiling start function
    ...
    public function edituser($eid = '', Request $request) {
        if (Auth::check()) {
            ...
            //dd($rec);   
            return view('admin.pc.deo.officer-profile', ['user_data' => $d, 'offrecords' => $rec]);
        } else {
            return redirect('/officer-login');
        }
    }
...
//         ->get();
               ...
               //dd($Percent_startdataentry);
               return view('admin.pc.deo.Expenditure.statusdashboard', ['user_data' => $d,
...
$st_code=!empty($st_code) ? $st_code : 0;
                ...
                if ($cons_no != '0') {
                    ...
                    $pendingCandList = DB::table('candidate_nomination_detail')
                                    ...
                                    ->where('candidate_nomination_detail.application_status', '=', '6')
                                    ->where('candidate_nomination_detail.finalaccepted', '=', '1')
                                    ->Where('candidate_personal_detail.cand_name', '<>', 'NOTA')->get();
                ...
                // dd(DB::getQueryLog());
                //dd($DataentryStartCandList);
                return view('admin.pc.deo.Expenditure.pending-report', ['user_data' => $d, 'pendingCandList' => $pendingCandList, 'edetails' => $ele_details, 'cons_no' => $cons_no, 'count' => count($pendingCandList)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
...
// end dataentry start function
    ...
    public function getpendingcandidateListgraph(Request $request, $pc) { 
    //dd($request->all());
        //PC ROPC candidateListBydataentryStart TRY CATCH STARTS HERE
        try {
            if (Auth::check()) {
...
$st_code=!empty($st_code) ? $st_code : 0;
                ...
                // dd(DB::getQueryLog());
                //dd($DataentryStartCandList);
                return view('admin.pc.deo.Expenditure.partiallypending-report', ['user_data' => $d, 'partiallyCandList' => $partiallyCandList, 'edetails' => $ele_details, 'cons_no' => $cons_no, 'count' => count($partiallyCandList)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
...
$st_code=!empty($st_code) ? $st_code : 0;
                ...
                // dd(DB::getQueryLog());
                //dd($DataentryStartCandList);
                return view('admin.pc.deo.Expenditure.defaulter-report', ['user_data' => $d, 'defaulterCandList' => $defaulterCandList, 'edetails' => $ele_details, 'cons_no' => $cons_no, 'count' => count($defaulterCandList)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function candidateListByfiledData(Request $request, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                //dd($DataentryStartCandList);
                return view('admin.pc.deo.Expenditure.filed-report', ['user_data' => $d, 'finalCandList' => $finalCandList, 'edetails' => $ele_details, 'cons_no' => $cons_no, "count" => count($finalCandList)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
...
$candidate_id=[];
    ...
    //dd($DataentryStartCandList);
    return view('admin.pc.deo.Expenditure.finalbyceo-report', ['user_data' => $d, 'finalbyceoCandList' => $finalbyceoCandList, 'edetails' => $ele_details, 'cons_no' => $cons_no, "count" => count($finalbyceoCandList)]);
} else {
    return redirect('/officer-login');
}
} catch (Exception $ex) {
...
public function getnoticeatCEO(Request $request,$pc){
    ...
    $noticeatCEO = DB::table('expenditure_reports')
        ...
        //dd($DataentryStartCandList);
        return view('admin.pc.deo.Expenditure.noticeatceo',['user_data' => $d,'noticeatCEO' => $noticeatCEO,'edetails'=>$ele_details,'st_code'=>$st_code,'cons_no'=>$cons_no,'count'=>count($noticeatCEO)]); 
...
////
    ...
    public function getTrackingByCEOUserId(Request $request) {
        ...
        if (Auth::check()) {
            ...
            return view('admin.pc.deo.Expenditure.tracking', ['user_data' => $d, 'ele_details' => $ele_details, "total_rec" => $total_rec, "cand_finalize_ro" => array(), "electionType" => $electionType, "expenditureData" => $data]);
        } else {
            return redirect('/officer-login');
        }
    }
...
/////////////////manish////////////
    ...
    public function getscrutinyreport(Request $request) {
        ...
        if (!empty($scrutinyReportData)) {
            return view('admin.pc.deo.Expenditure.GetScrutinyReport', compact('expensesourecefundbyitem', 'scrutinyReportData', 'expenseunderstated', 'expenseunderstatedbyitem'));
        } else {
...
//DB::enableQueryLog();
    ...
    public function getReturn(Request $request, $pc) {
        ...
        try {
			...
			->select('candidate_personal_detail.cand_name','expenditure_reports.*','m_party.CCODE', 'm_party.PARTYNAME')
                ...
                return view('admin.pc.deo.Expenditure.return-report', ['user_data' => $d, 'returnCandList' => $returnCandList ,
     ...
     public function getNonReturn(Request $request,$pc) {
        ...
        try {
			...
			->select('candidate_personal_detail.cand_name','expenditure_reports.*','m_party.CCODE', 'm_party.PARTYNAME')
                ...
                return view('admin.pc.deo.Expenditure.non-return-report', ['user_data' => $d, 'nonreturnCandList' => $nonreturnCandList ,
                    'edetails' => $ele_details, "count" => $count,
                     'st_code'=>$st_code,
                    'cons_no'=>$cons_no
                    ]);
    ...
    public function getcandidateList(request $request) {
        ...
        if (Auth::check()) {
            ...
            // dd(DB::getQueryLog());
            // dd($candList);
            return view('admin.pc.deo.Expenditure.FinalizedcandidateList', ['user_data' => $d, 'ele_details' => $ele_details, 'candList' => $candList]);
        } else {
            return redirect('/officer-login');
        }
    }
        ...
        try {
            if (Auth::check()) {
             ...
             // echo  $st_code.'cons_no=>'.$cons_no; die;
                ...
                return view('admin.pc.deo.Expenditure.mis-officer', ['user_data' => $d, 'totalContestedCandidatedata' => $totalContestedCandidatedata, 'st_code' => $st_code,'cons_no' => $cons_no, 'count' => count($totalContestedCandidatedata)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function getOfficersmisPDF(Request $request,$pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                $pdf = PDF::loadView('admin.pc.deo.Expenditure.mis-officerPDFhtml', ['user_data' => $d, 'totalContestedCandidatedata' => $totalContestedCandidatedata,'cons_no' => $cons_no, 'st_code' => $st_code]);    
                return $pdf->download('CEOPCMISPdf_' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
                return view('admin.pc.deo.Expenditure.mis-officerPDFhtml');
            } else {
                return redirect('/admin-login');
            }
        } catch (Exception $ex) {
    ...
    public function finalCandidateList(Request $request,$pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                //dd(DB::getQueryLog());
                // dd($totalContestedCandidate);
                return view('admin.pc.deo.Expenditure.candidate-report', ['user_data' => $d, 'totalContestedCandidatedata' => $totalContestedCandidatedata, 'cons_no' => $cons_no, 'st_code' => $st_code,'count' => count($totalContestedCandidatedata)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function finalCandidateListPDF(Request $request, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                $pdf = PDF::loadView('admin.pc.deo.Expenditure.candidatePDFhtml', ['user_data' => $d, 'totalContestedCandidatedata' => $totalContestedCandidatedata]);
                return $pdf->download('CeoCandidateMISPdf_' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
                return view('admin.pc.deo.Expenditure.candidatePDFhtml');
            } else {
                return redirect('/admin-login');
            }
        } catch (Exception $ex) {
                ...
                try {
                    if (Auth::check()) {
                        ...
                        if (empty($cons_no)) {
                         ...
                         //dd($startedcandidate);
                        return view('admin.pc.deo.Expenditure.mis-startedcandidate', ['user_data' => $d, 'startedcandidate' => $startedcandidate, 'st_code' => $st_code, 'cons_no' => $cons_no, 'count' => count($startedcandidate)]);
                    } else {
                        return redirect('/officer-login');
                    }
                } catch (Exception $ex) {
        ...
        try {
            if (Auth::check()) {
               ...
               if (empty($cons_no)) {
                ...
                $pdf = PDF::loadView('admin.pc.deo.Expenditure.mis-startedcandidatePdfhtml', ['user_data' => $d, 'filedData' => $filedData]);
                return $pdf->download('CeomiscandidatePdfhtml' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
                return view('admin.pc.deo.Expenditure.mis-startedcandidatePdfhtml');
            } else {
                return redirect('/admin-login');
            }
        } catch (Exception $ex) {
    ...
    public function getNotstartedMIS(Request $request, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                //  dd(DB::getQueryLog());
                return view('admin.pc.deo.Expenditure.mis-notstartedcandidate', ['user_data' => $d, 'notstarted' => $notstarted, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no, 'count' => count($notstarted)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function getNotstartedMISPDF(Request $request,$pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                $pdf = PDF::loadView('admin.pc.deo.Expenditure.mis-notstartedPdfhtml', ['user_data' => $d, 'notstarted' => $notstarted]);
                return $pdf->download('CeomisnotstartedPdfhtml' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
                return view('admin.pc.deo.Expenditure.mis-notstartedPdfhtml');
            } else {
                return redirect('/admin-login');
            }
        } catch (Exception $ex) {
    ...
    public function getfinalbyDEO(Request $request, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                // dd(DB::getQueryLog());
                return view('admin.pc.deo.Expenditure.finalbydeo-mis', ['user_data' => $d, 'finalbyDEO' => $finalbyDEO, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no, 'count' => count($finalbyDEO)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
...
//CEO getfinalbyDEOMISPDF PDF REPORT TRY CATCH BLOCK STARTS
        try {
            if (Auth::check()) {
                ...
                $pdf = PDF::loadView('admin.pc.deo.Expenditure.finalbyDEOPDFhtml', ['user_data' => $d, 'finalbyDEO' => $finalbyDEO]);
                return $pdf->download('CEOfinalbyDEOCandidateMIS_' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
                return view('admin.pc.deo.Expenditure.finalbyDEOPDFhtml');
            } else {
                return redirect('/admin-login');
            }
        } catch (Exception $ex) {
    ...
    if (empty($cons_no)) {
       ...
       // ->groupBy('expenditure_reports.candidate_id')
        ...
        $partiallyCandList = DB::table('candidate_nomination_detail')
                ...
                // dd(DB::getQueryLog());
                return view('admin.pc.deo.Expenditure.pendingatdeo-mis', ['user_data' => $d, 'partiallyCandList' => $partiallyCandList, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no, 'count' => count($partiallyCandList)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
...
$pdf = PDF::loadView('admin.pc.deo.Expenditure.candidatePendingatDEOPDFhtml', ['user_data' => $d, 'pendingatDEOCandList' => $pendingatDEOCandList]);
return $pdf->download('CeopendingatDEOCandidateMIS_' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
return view('admin.pc.deo.Expenditure.candidatePendingatDEOPDFhtml');
} else {
return redirect('/admin-login');
}
} catch (Exception $ex) {
...
/*$pendingatceoCandList = DB::table('expenditure_reports')
    ...
    return view('admin.pc.deo.Expenditure.pendingatceo-mis', ['user_data' => $d, 'pendingatceoCandList' => $pendingatceoCandList, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no, 'count' => count($pendingatceoCandList)]);
} else {
    return redirect('/officer-login');
}
} catch (Exception $ex) {
...
//dd($totalContestedCandidatedata);
$pdf = PDF::loadView('admin.pc.deo.Expenditure.candidatePendingatCEOPDFhtml', ['user_data' => $d, 'pendingatCEOCandList' => $pendingatCEOCandList]);
return $pdf->download('CeopendingatCEOCandidateMIS_' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
return view('admin.pc.deo.Expenditure.candidatePendingatCEOPDFhtml');
} else {
return redirect('/admin-login');
}
} catch (Exception $ex) {
...
//dd(DB::getQueryLog());
    //dd($notinTime);
    return view('admin.pc.deo.Expenditure.mis-notintimecandidate', ['user_data' => $d, 'notinTime' => $notinTime, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no, 'count' => count($notinTime)]);
} else {
    return redirect('/officer-login');
}
} catch (Exception $ex) {
...
if (Auth::check()) {
    ...
    $pdf = PDF::loadView('admin.pc.deo.Expenditure.mis-notintimecandidatePdfhtml', ['user_data' => $d, 'notinTime' => $notinTime]);
    return $pdf->download('CeomisnotintimecandidatePdfhtml' . trim($st_code) . '_Today_' . $cur_time . '.pdf');
    return view('admin.pc.deo.Expenditure.mis-notintimecandidatePdfhtml');
} else {
    return redirect('/admin-login');
}
} catch (Exception $ex) {
...
public function trackingReport(Request $request) {
    ...
    try {
        ...
        $d = $this->commonModel->getunewserbyuserid($user->id);
        $ele_details = $this->commonModel->election_details($d->st_code, $d->ac_no, $d->pc_no, $d->id, $d->officerlevel);
        $winn_data = DB::table('winning_leading_candidate')->select('leading_id', 'st_code', 'ac_no', 'nomination_id', 'candidate_id', 'trail_nomination_id', 'trail_candidate_id', 'lead_total_vote', 'trail_total_vote', 'margin', 'status', 'lead_cand_name', 'lead_cand_hname', 'lead_cand_party', 'lead_cand_hparty', 'trail_cand_name', 'trail_cand_hname', 'trail_cand_party', 'trail_cand_hparty')->where('st_code', $ele_details[0]->ST_CODE)->where('pc_no', $ele_details[0]->CONST_NO)->where('election_id', $ele_details[0]->ELECTION_ID)->first();
        $stcode = $d->st_code;
        $pc_no = $d->pc_no;
