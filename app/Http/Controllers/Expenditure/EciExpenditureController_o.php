            ...
            if (Auth::check()) {
               ...
               $sql = DB::table('candidate_nomination_detail')
                ...
                if (!empty($st_code) && empty($cons_no)) {
                    $sql = $sql->where('candidate_nomination_detail.st_code', '=', $st_code);
                    $totalContestedCandidate = $sql->count();
                   ...
                   $sql = $sql->where('candidate_nomination_detail.st_code', '=', $st_code);
                   $sql = $sql->where('candidate_nomination_detail.pc_no', '=', $cons_no);
                   $totalContestedCandidate = $sql->count();
                    ...
                    $totalContestedCandidate = $sql->count();
                    $totalElectedCandidate = DB::table('winning_leading_candidate')->count();
                }
                ...
                //Get logged Count 
                $logedaccount = $this->eciexpenditureModel->gettotallogedAccount('Yes', $st_code, $cons_no);
                //Get logged Count %
                $Percent_logedaccount = $this->get_percentage($totalContestedCandidate, $logedaccount);

                //Get logged Count 
                $Notlogedaccount = $this->eciexpenditureModel->gettotallogedAccount('No', $st_code, $cons_no);
                //Get logged Count %
                $Percent_Notlogedaccount = $this->get_percentage($totalContestedCandidate, $Notlogedaccount);
                ...
                $notinmannercount = $this->eciexpenditureModel->getnatureofDefAccount(3, $st_code, $cons_no);
                //Get Defects in format Count %
                $Percent_notinmannercount = $this->get_percentage($totalContestedCandidate, $notinmannercount);
              ...
              //  $Percent_formateDefectscount = $this->get_percentage($totalContestedCandidate, $formateDefectscount);
                ...
                //Get expunderstatedReturn in format Count 
                $expunderstatedReturn = $this->eciexpenditureModel->gettotalexpenseUnderStated('Returned', $st_code, $cons_no);
                //Get expunderstatedReturn in format Count %
                $Percent_expunderstatedReturn = $this->get_percentage($totalContestedCandidate, $expunderstatedReturn);

                 //Get expunderstatedNonReturn in format Count 
                $expunderstatedNonReturn = $this->eciexpenditureModel->gettotalexpenseUnderStated('Non-Returned', $st_code, $cons_no);
                //Get expunderstatedNonReturn in format Count %
                $Percent_expunderstatedNonReturn = $this->get_percentage($totalContestedCandidate, $expunderstatedNonReturn);
                ...
                //shishir
/////////////////-----------end notification -----------------//////////////
    return view('admin.pc.eci.Expenditure.dashboard', ['user_data' => $d, 
        ...
        'notinmannercount' => $notinmannercount,'Percent_notinmannercount' => $Percent_notinmannercount, 
        'expunderstatedReturn' => $expunderstatedReturn,'Percent_expunderstatedReturn' => $Percent_expunderstatedReturn, 
         'expunderstatedNonReturn' => $expunderstatedNonReturn,'Percent_expunderstatedNonReturn' => $Percent_expunderstatedNonReturn, 
         'Percent_partyFund' => $Percent_partyFund,'Percent_OthersourcesFund' => $Percent_OthersourcesFund, 
         'edetails' => $ele_details,
         'logedaccount' => $logedaccount,'Percent_logedaccount' => $Percent_logedaccount,
         'Notlogedaccount' => $Notlogedaccount,'Percent_Notlogedaccount' => $Percent_Notlogedaccount,
         'notintimeaccount' => $notintimeaccount, 'Percent_notintimeaccount' => $Percent_notintimeaccount,
            ...
            if (Auth::check()) {
                ...
                if (count($totalState) > 0) {
                    foreach ($totalState as $item) {
                        ...
                        $datestart = $this->eciexpenditureModel->gettotaldataentryStartdataeci('PC', $item->state_code);
        ...
        try {
            if (Auth::check()) {
                ...
                $sql=DB::table('expenditure_reports')
                            ...
                            ->Where('candidate_personal_detail.cand_name', '<>', 'NOTA')
                            ->groupBy('expenditure_reports.candidate_id')
                              ->orderBy('expenditure_reports.ST_CODE','ASC')
                            ->orderBy('expenditure_reports.constituency_no','ASC')
                            ->orderBy('candidate_personal_detail.cand_name','ASC');
                if ($st_code == '0' && $cons_no == '0') {
                    $finalCandList = $sql->get();
                } elseif ($st_code != '0' && $cons_no == '0') {
...
// end candidateListByfinalizeData start function
            ...
            if (Auth::check()) {
                ...
                if (count($totalState) > 0) {
                    foreach ($totalState as $item) {
                        ...
                        $datestart = $this->eciexpenditureModel->gettotaldataentryFinaldataeci('PC', $item->state_code);
                        if (!empty($datestart[0]) && $datestart[0]->state_code == $item->state_code) {
     ...
     */
    public function candidateListBylogedaccount(Request $request, $state, $pc,$logged) { 
        ...
        try {
            if (Auth::check()) {
                ...
                $st_code = base64_decode($xss->clean_input($state));
                $cons_no = base64_decode($xss->clean_input($pc));
                $logged = base64_decode($xss->clean_input($logged));
                ...
                $sql= DB::table('expenditure_reports')
                    ...
                    ->leftjoin('candidate_personal_detail', 'candidate_personal_detail.candidate_id', '=', 'expenditure_reports.candidate_id')
                    ->select('expenditure_reports.created_at', 'expenditure_reports.final_by_ro', 'expenditure_reports.candidate_id', 'expenditure_reports.ST_CODE', 'expenditure_reports.constituency_no', 'expenditure_reports.last_date_prescribed_acct_lodge', 'candidate_personal_detail.candidate_id', 'candidate_personal_detail.cand_name', 'candidate_nomination_detail.candidate_id', 'candidate_nomination_detail.application_status', 'candidate_nomination_detail.finalaccepted', 'm_party.CCODE', 'm_party.PARTYNAME')
                    ->where('expenditure_reports.candidate_lodged_acct', '=', $logged)
                    ->where('candidate_nomination_detail.application_status', '=', '6')
                    ->where('candidate_nomination_detail.finalaccepted', '=', '1')
                    ->where('candidate_nomination_detail.symbol_id', '<>', '200')
                    ->Where('candidate_personal_detail.cand_name', '<>', 'NOTA')
                ...
                $title= $logged='Yes' ? 'Logged Account Candidates' : 'Not Logged Account Candidates';
                return view('admin.pc.eci.Expenditure.logedaccount-report', ['user_data' => $d, 'logedAccount' => $logedAccount, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no, 'title' =>$title,'count' => count($logedAccount)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
...
// end candidateListBynotintime start function
            ...
            if (Auth::check()) {
                ...
                if (count($totalState) > 0) {
                    foreach ($totalState as $item) {
                        ...
                        $datestart = $this->eciexpenditureModel->gettotalNotinTimeeci('PC', $item->state_code);
                        if (!empty($datestart[0]) && $datestart[0]->state_code == $item->state_code) {
    ...
    public function candidateListByformatedefects(Request $request, $state, $pc) {
        ...
        try {
            ...
            return Redirect('/internalerror')->with('error', 'Internal Server Error');
        }//PC ECI candidateListByformatedefects TRY CATCH ENDS HERE   
    ...
    public function candidateListByformatedefectsgraph(Request $request, $state, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                if (count($totalState) > 0) {
                    foreach ($totalState as $item) {
                        $datestart = $this->eciexpenditureModel->gettotalformatedefectsdata('PC', $item->state_code);
                        if (!empty($datestart[0]) && $datestart[0]->state_code == $item->state_code) {
 ...
 /**
     * @author Devloped By : Niraj Kumar
     * @author Devloped Date : 19-08-2020
     * @author Modified By : 
     * @author Modified Date : 
     * @author param return getCandbyDefAcc By ECI fuction     
     */
    public function getCandbyDefAcc(Request $request, $state, $pc,$def_type) {

        //PC ECI getCandbyDefAcc TRY CATCH STARTS HERE
        try {
            if (Auth::check()) {
                $user = Auth::user();
                $uid = $user->id;
                ...
                $st_code = base64_decode($xss->clean_input($state));
                $cons_no = base64_decode($xss->clean_input($pc));
                $def_type = base64_decode($xss->clean_input($def_type));
                $st_code = !empty($st_code) ? $st_code : 0;
                $cons_no = !empty($cons_no) ? $cons_no : 0;
                $defaultaccount = 0;
                DB::enableQueryLog();
                $sql=DB::table('expenditure_reports')
                            ...
                            ->leftjoin('candidate_personal_detail', 'candidate_personal_detail.candidate_id', '=', 'expenditure_reports.candidate_id')
                            ->select('expenditure_reports.created_at', 'expenditure_reports.final_by_ro', 'expenditure_reports.candidate_id', 'expenditure_reports.ST_CODE', 'expenditure_reports.constituency_no', 'expenditure_reports.last_date_prescribed_acct_lodge', 'candidate_personal_detail.candidate_id', 'candidate_personal_detail.cand_name', 'candidate_nomination_detail.candidate_id', 'candidate_nomination_detail.application_status', 'candidate_nomination_detail.finalaccepted', 'm_party.CCODE', 'm_party.PARTYNAME')
                            ->where('expenditure_reports.nature_of_default_ac', '=', $def_type)
                            ->where('candidate_nomination_detail.application_status', '=', '6')
                            ->where('candidate_nomination_detail.finalaccepted', '=', '1')
                            ->where('candidate_nomination_detail.symbol_id', '<>', '200')
                            ->Where('candidate_personal_detail.cand_name', '<>', 'NOTA')
                ...
                if ($st_code == '0' && $cons_no == '0') {
                  ...
                  //  dd(DB::getQueryLog());
                 $title=($def_type==3) ? 'NOT LOGGED IN MANNER' : '';
                //dd($defaultaccount);
                return view('admin.pc.eci.Expenditure.defaultaccount-report', ['user_data' => $d, 'defaultaccount' => $defaultaccount, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no, 'count' => count($defaultaccount),'title'=>$title]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
            return Redirect('/internalerror')->with('error', 'Internal Server Error');
        }//PC ECI getCandbyDefAcc TRY CATCH ENDS HERE   
    } // end getCandbyDefAcc start function
...
// end candidateListByronotagree start function
     ...
     */
    public function candidateListByunderstatedexpense(Request $request, $state, $pc,$return_type) {
        ...
        try {
            if (Auth::check()) {
                ...
                $st_code = base64_decode($xss->clean_input($state));
                $cons_no = base64_decode($xss->clean_input($pc));
                $return_type = base64_decode($xss->clean_input($return_type));
                $st_code = !empty($st_code) ? $st_code : '';
                $cons_no = !empty($cons_no) ? $cons_no : 0;
                 DB::enableQueryLog();
                  $sql=DB::table('expenditure_understates')
                  ->join('expenditure_reports', 'expenditure_reports.candidate_id', '=', 'expenditure_understates.candidate_id')
                  ...
                  ->where('expenditure_reports.return_status',$return_type)
                  ->where('expenditure_understates.status','no')
                 ->select('expenditure_reports.created_at', 'expenditure_reports.candidate_id', 'expenditure_reports.ST_CODE as st_code', 'expenditure_reports.constituency_no as pc_no','candidate_personal_detail.candidate_id', 'candidate_personal_detail.cand_name', 'candidate_nomination_detail.candidate_id', 'expenditure_reports.final_by_ro', 'm_party.CCODE', 'm_party.PARTYNAME')
                  ->groupBy('expenditure_understates.candidate_id')
                  ->orderBy('candidate_nomination_detail.st_code','ASC')
                  ->orderBy('candidate_nomination_detail.pc_no','ASC')
                  ->orderBy('candidate_personal_detail.cand_name','ASC');
                if ($st_code == '0' && $cons_no == '0') {

                    $expenseunderstated = $sql->get();

                   } elseif ($st_code != '0' && $cons_no == '0') {

                  $sql=$sql->where('expenditure_understates.ST_CODE', '=', $st_code);
                  $expenseunderstated = $sql->get();

                 } elseif ($st_code != '0' && $cons_no != '0') {
                    $sql=$sql->where('expenditure_understates.ST_CODE', '=', $st_code);
                    $sql=$sql->where('expenditure_understates.constituency_no', '=', $cons_no);
                    $expenseunderstated = $sql->get();
                }
                 //dd(DB::getQueryLog());
                //dd($expenseunderstated);
                return view('admin.pc.eci.Expenditure.expenseunderstated-report', ['user_data' => $d, 'expenseunderstated' => $expenseunderstated, 'edetails' => $ele_details, 'st_code' => $st_code, 'cons_no' => $cons_no,'title' => $return_type, 'count' => count($expenseunderstated)]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
            ...
            if (Auth::check()) {
                ...
                if (count($totalState) > 0) {
                    foreach ($totalState as $item) {
                        ...
                        $datestart = $this->eciexpenditureModel->gettotalexpenseUnderStateddataeci('PC', $item->state_code);
                        if (!empty($datestart[0]) && $datestart[0]->state_code == $item->state_code) {
            ...
            if (Auth::check()) {
                ...
                if (count($totalState) > 0) {
                    foreach ($totalState as $item) {
                        ...
                        $datestart = $this->eciexpenditureModel->gettotalPartyfunddataeci('PC', $item->state_code);
                        $otherfund = $this->eciexpenditureModel->gettotalOtherSourcesfunddata('PC', $item->state_code);
                        if (!empty($datestart[0]) && $datestart[0]->state_code == $item->state_code) {
    ...
    public function candidateListByothersfund(Request $request, $state, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                $query->select('candidate_personal_detail.cand_name', 'candidate_personal_detail.candidate_id', 'candidate_personal_detail.candidate_father_name', DB::raw('IFNULL(sum(expenditure_fund_source.other_source_amount),0 )as other_source_amount'), 'm_party.CCODE', 'm_party.PARTYNAME', 'expenditure_fund_source.ST_CODE', 'expenditure_fund_source.constituency_no', 'expenditure_reports.final_by_ro');
    ...
    public function getCandidatesmisEXL(Request $request, $state, $pc) {
        ...
        try {
            if (Auth::check()) {
                ...
                return \Excel::download / Excel::store($arr);
            } else {
                return redirect('/admin-login');
            }
        } catch (Exception $ex) {
            ...
            if (Auth::check()) {
                ...
                if ($st_code == '0' && $cons_no == '0') {
                    ...
                    $pendingCandList = DB::table('candidate_nomination_detail')
                            ...
                            ->orderBy('candidate_nomination_detail.st_code','ASC')
                                    ->orderBy('candidate_nomination_detail.pc_no','ASC')
                                    ->orderBy('candidate_personal_detail.cand_name','ASC')->get();
                } elseif ($st_code != '0' && $cons_no != '0') {
                    $startCandList = DB::table('expenditure_reports')->select('candidate_id')
                            ->where('expenditure_reports.ST_CODE', '=', $st_code)
                            ->where('expenditure_reports.constituency_no', '=', $cons_no)
...
//  dd($conditions);
               ...
               resultunderstated'), function($join) {
...
//->where('expenditure_reports.final_by_ro', '=', '1');
                ...
                $query->orderBy('expenditure_reports.ST_CODE','ASC');
                $query->orderBy('expenditure_reports.constituency_no','ASC');
                $query->orderBy('candidate_personal_detail.cand_name','ASC');
...
/////by manish
    ...
    public function confirmReport() {
        ...
        // SMS and Email Setting start
        $st_code = 'S01';
        $ECIDetails = DB::table('expenditure_zone_users')->select('email', 'phone_no')
                        ->whereRaw("find_in_set('$st_code',assign_state)")
                        ->where('role_id', 28)->first();
        //dd($ECIDetails);
        // $email = !empty($CEODetails->email)?$CEODetails->email:'';
        //$phone_no = !empty($CEODetails->phone_no)?$CEODetails->phone_no:'';
        $email = 'manojk.eci07@gmail.com';
        $phone_no = '9667970724';
        $message = "Candidate Name:Manoj Kumar\n"
                . "PC Name:West Delhi\n"
                . "State:Delhi\n"
                . "Election Type:PC\n"
                . "Election Year:2019\n"
                . "Candidate Scrutiny Report has been submitted successfully.";


        // SMS and Email Setting end
        if ($insertComment) {
            $this->expenditureModel->sendSMS($phone_no, $message);
            $this->expenditureModel->sendEmail($email, $message);
            $smsemailInsert = DB::table('expenditure_sms_email_status')->insert([
                'candidate_id' => $candidate_id, 'constituency_no' => $constituency_no, 'ST_CODE' => $st_code,
                'is_sms' => $smsStatus, 'is_email' => $emailStatus, 'election_id' => $election_id, 'created_at' => date('Y-m-d h:i:s'),
                'created_by' => $uid, 'updated_at' => date('Y-m-d h:i:s'), 'updated_by' => $uid]);
...
////
            ...
            $pdf = view('admin.expenditure.pdf_ro', compact('expensesourecefundbyitem', 'scrutinyReportData', 'districtDetails', 'expenseunderstated', 'expenseunderstatedbyitem', 'submitedData', 'electionType', 'download_link1', 'electionType', 'download_link2', 'download_link3', 'download_link4', 'winn_data', 'partyname'));
            $mpdf->WriteHTML($pdf);
            $mpdf->Output();
        } else {
            return redirect('/officer-login');
...
/////manish
    ...
    public function editExpenditureReport(Request $request) {
        if (Auth::check()) {
            ...
            $atrs = $this->expenditureModel->getATRData($candidate_id);
    ...
    public function StoreMisExpenseReport(Request $request) {
        ...
        $election_id = !empty($ReportSingleData[0]->election_id) ? $ReportSingleData[0]->election_id : 0;
        $st_code = !empty($ReportSingleData[0]->ST_CODE) ? $ReportSingleData[0]->ST_CODE : 0;
        $constituency_no = !empty($ReportSingleData[0]->constituency_no) ? $ReportSingleData[0]->constituency_no : 0;
        try {
            $data_arr = array();
            foreach ($request as $key => $req_data) {
                $xss = new xssClean;
            ...
            $unsetItems = ['candidate_id', 'constituency_no', 'constituency_nos', 'contensting_candiate',
                'date_of_declaration', 'user_id', 'notice_send_to', 'final_action_taken', 'final_action_date'];

            $dataUpdate = array_diff_key($data_arr, array_flip($unsetItems));
            $final_action_taken = $request['final_action_taken'];
            $final_action_date = $request['final_action_date'];
            if (in_array($dataUpdate['final_action'], ['Reply Issued By ECI', 'Notice Issued By ECI'])) {
                $dataUpdate['final_action'] = $final_action_taken;
                $dataUpdate['final_by_eci'] = 0;
            }

            $updateStatus = DB::table('expenditure_reports')->where('candidate_id', $candidate_id)->update($dataUpdate);
            if ($final_action == 'Closed' || $final_action == 'Disqualified') {
            ...
            // for sms and email for notice and reply case to ceo start
            if (!empty($request['final_action_taken']) && in_array($request['final_action_taken'], ['Notice Issued', 'Reply Issued'])) {

                $candidatedetails = DB::table('candidate_nomination_detail')
                        ->join('candidate_personal_detail', 'candidate_personal_detail.candidate_id', '=', 'candidate_nomination_detail.candidate_id')
                        ->select('candidate_personal_detail.cand_name', 'candidate_nomination_detail.pc_no')
                        ->where('candidate_nomination_detail.st_code', $st_code)
                        ->where('candidate_nomination_detail.election_id', $election_id)
                        ->where('candidate_nomination_detail.application_status', '=', '6')
                        ->where('candidate_nomination_detail.party_id', '<>', '1180')
                        ->where('candidate_nomination_detail.finalaccepted', '=', '1')
                        ->where('candidate_nomination_detail.candidate_id', '=', $candidate_id)
                        ->first();
                $candiateName = !empty($candidatedetails->cand_name) ? $candidatedetails->cand_name : 0;
                $pc_no = !empty($candidatedetails->pc_no) ? $candidatedetails->pc_no : 0;
                $pcDetails = getpcbypcno($st_code, $pc_no);
                $pcName = !empty($pcDetails->PC_NAME) ? $pcDetails->PC_NAME : '';

                $stateDetails = getstatebystatecode($st_code);
                $stateName = !empty($stateDetails->ST_NAME) ? $stateDetails->ST_NAME : '';

                $d = $this->commonModel->getunewserbyuserid($user->id);
                $ele_details = $this->commonModel->election_details($d->st_code, $d->ac_no, $d->pc_no, $d->id, $d->officerlevel);

                $ELECTION_TYPE = !empty($ele_details[0]->ELECTION_TYPE) ? $ele_details[0]->ELECTION_TYPE : '';
                $YEAR = !empty($ele_details[0]->ELECTION_TYPE) ? $ele_details[0]->YEAR : '';
                $CONST_TYPE = !empty($ele_details[0]->ELECTION_TYPE) ? $ele_details[0]->CONST_TYPE : '';
                $ECIDetails = DB::table('officer_login')
                                ->select('phone_no', 'email')
                                ->where('st_code', $st_code)
                                ->where('pc_no', $pc_no)
                                ->where('officer_login.role_id', 18)->first();
                // dd($ECIDetails);
                // $email = !empty($CEODetails->email)?$CEODetails->email:'';
                //$phone_no = !empty($CEODetails->phone_no)?$CEODetails->phone_no:'';
                $email = 'manojk.eci07@gmail.com';
                $phone_no = '9667970724';
                $message = "Candidate Name:" . $candiateName . "\n"
                        . "PC Name:" . $pcName . "\n"
                        . "State:" . $stateName . "\n"
                        . "Election Type:" . $CONST_TYPE . "\n"
                        . "Election Year:" . $YEAR . "\n"
                        . "Status:" . $final_action_taken . "\n"
                        . "Candidate Scrutiny Report has been submitted By ECI.";
                //$this->expenditureModel->sendSMS($phone_no,$message); 
                // $this->expenditureModel->sendEmail($email,$message);
                // for sms and email for notice and reply case to ceo start
                // Action taken Report start here
                ////////////////////////////////// add entry in expenditure action logs/////////////////

                $cdate = date('Y-m-d h:i:s');

                $data_action = array("candidate_id" => $candidate_id,
                    "final_action_taken" => $final_action_taken,
                    "final_action_date" => $final_action_date,
                    "comment" => $comment_by_eci,
                    "role_id" => $role_id,
                    "created_by" => $uid);
                ...
                $check_exits_log = DB::table('expenditure_action_logs')
                        ->where('ST_CODE', $st_code)
                        ->where('constituency_no', $constituency_no)
                        ->where('election_id', $election_id)
                        ->where('candidate_id', $candidate_id)
                        ->where('role_id', $role_id)
                        ->where('final_action_date', $final_action_date)
                        ->first();

                $data_arr_action['final_action_date'] = $final_action_date;

                $data_arr_action['election_id'] = $election_id;
                $data_arr_action['updated_at'] = date('Y-m-d H:i:s');
                $data_arr_action['updated_by'] = $uid;

                if (!empty($check_exits_log)) {
                    $data_actionInserted = DB::table('expenditure_action_logs')
                            ->where('ST_CODE', $st_code)
                            ->where('constituency_no', $constituency_no)
                            ->where('election_id', $election_id)
                            ->where('candidate_id', $candidate_id)
                            ->where('final_action_taken', $final_action_taken)
                            ->where('final_action_date', $final_action_date)
                            ->update($data_arr_action);
                } else {
                    $smsStatus = $this->expenditureModel->sendSMS($phone_no, $message);
                    $emailStatus = $this->expenditureModel->sendEmail($email, $message);
                    $data_arr_action['election_id'] = $election_id;
                    $data_arr_action['constituency_no'] = $constituency_no;
                    $data_arr_action['ST_CODE'] = $st_code;
            ...
            // Action Taken Report end

            if ($updateStatus > 0) {
                Session::put('message', "Saved successfully");
                return redirect($namePrefix . '/editExpenditureReport?candidate_id=' . base64_encode($candidate_id))->withInput();
            } else {
    ...
    public function GetProfileECI(Request $request) {
        try {
            if (Auth::check()) {
                ...
                return view('admin.expenditure.GetProfileRO', compact('profileData', 'ReportSingleData', 'electionType', 'nature_of_default_ac', 'current_status'));
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function printTrackingStatus($candidateId) {
        if (Auth::check()) {
            ...
            $pdf = view('admin.expenditure.pdf_ro_tracking', compact('profileData', 'ReportSingleData', 'electionType', 'nature_of_default_ac', 'current_status'));
            $mpdf->WriteHTML($pdf);
            $mpdf->Output();
    ...
    public function candidate_wise_expenditure(Request $request) {
        ...
        if (Auth::check()) {
            ...
            if (!empty($conditions)) {
              ...
              order by TEMP.grand_total_election_exp_by_cadidate desc");
            } else {
...
/////////////////////////// For Timeline Start ///////////////////////////////////
    public function getATRTimeLine1(Request $request) {
        try {
            if (Auth::check()) {
                $user = Auth::user();
                $d = $this->commonModel->getunewserbyuserid($user->id);
                $ele_details = $this->commonModel->election_details($d->st_code, $d->ac_no, $d->pc_no, $d->id, $d->officerlevel);
                return view('admin.pc.eci.Expenditure.atrtimeline1', ['user_data' => $d]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
    ...
    public function getATRTimeLine2(Request $request) {
        try {
            if (Auth::check()) {
                $user = Auth::user();
                $d = $this->commonModel->getunewserbyuserid($user->id);
                $ele_details = $this->commonModel->election_details($d->st_code, $d->ac_no, $d->pc_no, $d->id, $d->officerlevel);
                return view('admin.pc.eci.Expenditure.atrtimeline2', ['user_data' => $d]);
            } else {
                return redirect('/officer-login');
            }
        } catch (Exception $ex) {
