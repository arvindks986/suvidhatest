  ...
  */
  ...
  public function get_report(Request $request){  
    ...
    //end validate request
      ...
      $data['action'] = url('pcceo/report/scrutiny');
    ...
    $data['downlaod_to_excel'] = url('pcceo/report/scrutiny/excel').'?'.implode('&', $request_array);
    $data['downlaod_to_pdf']   = url('pcceo/report/scrutiny/pdf').'?'.implode('&', $request_array);
    ...
    return view('admin.pc.ceo.report.date_wise_report', $data);   
  }    
  ...
  public function detail($id,Request $request){
    ...
    $data['heading_title'] = "Scrutiny Report of ".$status_name." candidate(s)";
      ...
      $data['constituency'] = NULL;
      if($request->has('constituency')){
        $data['constituency'] = (int)$request->constituency;
        $request_array[] = 'constituency='.$request->constituency;
      }

      $data['phase'] = NULL;
      if($request->has('phase')){
        $data['phase'] = (int)$request->phase;
        $request_array[]  = 'phase='.$request->phase;
      }
      ...
      foreach ($candidates as $candidate) {
        ...
        $const=$this->commonModel->getpcname($candidate->ST_CODE,$candidate->PC_NO);  
          ...
          $results[] = [
            ...
            'href'           => url('pcceo/report/scrutiny/detail-by-nomination/'.base64_encode($candidate->nomination_id))
          ]; 
        $index++;
      }
    ...
    return view('admin.pc.ceo.report.date_wise_report_name', $data);     
  }  
  ...
  public function pdf(Request $request){
    $data = $this->get_report($request->merge(['is_excel' => 1]));
    $pdf = \PDF::loadView('admin.pc.ceo.report.pdf',$data);
