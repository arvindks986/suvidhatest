...
class ApplicantController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $result=NominationApplicationModel::select('nomination_application.*', 'dist_pc_mapping.DIST_NO as DIST_NO_HDQTR','dist_pc_mapping.DIST_NAME_EN as DIST_NAME')->where('nomination_application.st_code', $ele_details->ST_CODE)
        ->join('dist_pc_mapping', [['nomination_application.st_code', '=', 'dist_pc_mapping.ST_CODE'], ['nomination_application.pc_no', '=', 'dist_pc_mapping.PC_NO']])
        ...
        ->groupBy('nomination_application.nomination_no')
        ->get()
        ->toarray();
...
public function is_nomination_exist($nom_no){
    $user = Auth::user();
    $d=$this->commonModel->getunewserbyuserid($user->id);  
    $ele_details=$this->commonModel->election_details($d->st_code,$d->pc_no,$d->dist_no,$d->id,'PC');
...
{   
    ...
    if($check_finalize->finalized_ac=='1') 
    {
        \Session::flash('error_mes', 'PC is Finalized');
        //return Redirect::to('/ropc/dashboard');
    }
    $seched=getschedulebyid($ele_details->ScheduleID);
    ...
    $data['pan_number'] = $this->decrypt_pan($data['pan_number']); //dd($data);
...
$data['heading_title']="Candidte Information";
  ...
  public function decisionbyro(Request $request) {
            ...
            if(isset($shares)){
				...
				$data['nom_details'] = $temp_nom_data;
                $data['heading_title']="Candidte Information";
                return view($this->view_path.'.decisionbyro', $data); 
...
{ 
    ...
    if ($validator->fails()) { 
      //  return redirect('/ropc/decisionvalidatel')
        return Redirect::back()
        ->withErrors($validator)
        ->withInput();
    }
    ...
    $pc=getpcbypcno($shares->st_code,$shares->pc_no);
    $state=getstatebystatecode($shares->st_code);

    $sub="Your Nomination Application Status change";

    $Mob_otp="Dear ".$shares->cand_name." your Nomination Application of the constituency ".$pc->PC_NAME." of state ".$state->ST_NAME." is Receipt Generated by returning officer";

    $html ="<html>
    <body>
    <p>Dear ".$shares->cand_name.",<br/><br/></p>
    <p>Your Nomination Application Serial No. of nomination paper <b> ".$shares->nomination_papersrno."</b> ,  Form is Submitted on date ".date("d/m/Y")." for the General / Bye elections of the constituency <b> ".$pc->PC_NAME."</b> of state  <b> ".$state->ST_NAME."</b>. Your Scutiny Date <b> ".date("d-m-Y",strtotime($shares->scrutiny_date))."</b> and time <b> ".$shares->scrutiny_time."</b> is <b>Receipt Generated </b>by returning officer</p>
    <p><br/><br/>Regards,<br/>
...
//QR code
    ...
    $this->upload_folder = config("public_config.upload_folder");
    $destination_path = FileModel::get_file_path($this->upload_folder.'/qrcode/'.$year.'/ac/'.$election_name.'/'.$st_code.'/'.$pc_no).'/'.$data['id'].'.png';
