   ...
   public function __construct()
    ...
    public function index()
	    ...
	    if(Auth::check()){
            ...
            return view('admin.pc.ro.nomination.dashboard', $data);	            
	        }
	        else {
	              return redirect('/officer-login');
	        	  }
	...
	public function listallcandidate(Request $request)
           ...
           $sechdul=checkscheduledetails($seched);
		        ...
		        $val=$this->romodel->checkfinalize_acbyro($d->st_code,$d->ac_no,$d->officerlevel);
		     	...
		     	$status=allstatus();
		     	//dd($list);
	        return view('admin.pc.ro.listallcandidate', ['user_data' => $d,'cand_finalize_ceo' =>$check_finalize->finalize_by_ceo,'cand_finalize_ro' =>$check_finalize->finalized_ac,'sechdul' => $sechdul,'sched'=>$seched,'lists'=>$list,'status'=>$cand_status,'checkval'=>$val, 'status_list'=>$status,'edetails'=>$ele_details]);
		         }
	        else {
	              return redirect('/officer-login');
	        	  }
    ...
    public function withdrawn_candidates(Request $request)
	        ...
	        return view('admin.pc.ro.withdrawn_candidates', ['user_data' => $d,'cand_finalize_ceo' =>$check_finalize->finalize_by_ceo,'cand_finalize_ro' =>$check_finalize->finalized_ac,'sechdul' => $sechdul,'sched'=>$seched,'lists'=>$list,'status'=>$cand_status,'checkval'=>$val, 'status_list'=>$status,'edetails'=>$ele_details]);
		         }
	        else {
	              return redirect('/officer-login');
	        	  }
	...
	public function accepted_application(Request $request)
			...
			if(Auth::check()){
	         ...
	         if(isset($ele_details))
		    	...
		    	return view('admin.pc.ro.listaccepted',['user_data' => $d,'cand_finalize_ceo' =>$check_finalize->finalize_by_ceo,'cand_finalize_ro' =>$check_finalize->finalized_ac,'sechdul' => $sechdul,'sched'=>$seched,'lists'=>$list,'checkval'=>$val,'edetails'=>$ele_details]);
		         }
	        else {
		    ...
		    //->where('candidate_nomination_detail.finalize','=','1')
		     ->Where('candidate_personal_detail.cand_name', '<>', 'NOTA')
		      ->where(function($query1) use ($a2,$a3){
                    		$query1->where('m_party.PARTYTYPE','=',$a2)
                   			->orWhere('m_party.PARTYTYPE','=',$a3);
              			})
		    ->orderBy('candidate_nomination_detail.new_srno', 'asc')
    		->select('candidate_personal_detail.cand_name','candidate_personal_detail.candidate_residence_address','candidate_nomination_detail.*', 'm_party.PARTYNAME','m_party.PARTYABBRE','m_party.PARTYTYPE','m_symbol.SYMBOL_DES','candidate_personal_detail.candidate_residence_address','candidate_personal_detail.candidate_residence_stcode','candidate_personal_detail.candidate_residence_districtno','candidate_personal_detail.candidate_residence_acno')->get(); 
		   ...
		   // print_r($cands); print_r($candu);  print_r($candz); die;
              ...
              // view()->share('candn',$candn,'st',$state,'pc',$pc);
		        ...
		        if($request->has('download')){
		            $pdf = PDF::loadView('admin.pdfview',compact('candn',$candn,'cands',$cands,'candu',$candu,'candz',$candz,'state',$state,'pc',$pc));
			...
			if(Auth::check()){
	         ...
	         if(isset($ele_details))
		            $seched=getschedulebyid($ele_details->ScheduleID);
		            	...
		            	$seched='';
		           $sechdul=checkscheduledetails($seched);
				/*  $list1=$this->romodel->Symbolcandidate($ele_details);
	            foreach ($list1 as  $val) {
	            	$affidavitcheck=getById('candidate_affidavit_detail','nom_id',$val->nom_id);
	            	if(!isset($affidavitcheck)){
	            	 $st = array('rejection_message'=>"No Affidavit uploaded",'application_status'=>4);
	            	  $i = DB::table('candidate_nomination_detail')->where('nom_id', $val->nom_id)->update($st); 
	            	}
               ...
               $sym = DB::table("m_symbol")->whereNOTIn('SYMBOL_NO',function($query){
		    	...
		    	return view('admin.pc.ro.symboldetails',['user_data' => $d,'cand_finalize_ceo' =>$check_finalize->finalize_by_ceo,'cand_finalize_ro' =>$check_finalize->finalized_ac,'sechdul' => $sechdul,'sched'=>$seched,'lists'=>$list,'checkval'=>$val,'edetails'=>$ele_details,'sym'=>$sym]);
		         }
	        else {
	         	return Redirect::to('/officer-login');
	        	  }	
	...
	function finalize_ac()
        ...
        $check_ac = DB::table('candidate_finalized_ac')->where('st_code',$ele_details->ST_CODE)
        	...
        	$currentTime = $date->format('Y-m-d H:i:s');
		      ...
		      $mob_message = "Dear Sir/Madam Your OTP is ".$otp." for finalized PC in Suvidha Portal.Please enter the OTP  to proceed.This OTP is valid only for 10 minutes.Do not share this OTP. Regards  
		          Team ICT  ";
...
///$message   = Crypt::decrypt($encrypted);
           ...
           // dd($request);
            ...
            return view('admin.pc.ro.listnomination', ['user_data' => $d,'cand_finalize_ceo' =>$check_finalize->finalize_by_ceo,'cand_finalize_ro' =>$check_finalize->finalized_ac,'sechdul' => $sechdul,'sched'=>$seched, 'lists'=>$list,'status'=>$cand_status,'checkval'=>$val,'status_list'=>$status,'edetails'=>$ele_details]);
        }
        else {
            return redirect('/officer-login');
        }
				...
				if(Auth::check()){ 
		         ...
		         if(isset($ele_details))
		           ...
		           $sechdul=checkscheduledetails($seched);
					...
					return view('admin.pc.ro.viewnomination', ['user_data' => $d,'cand_finalize_ceo' =>$check_finalize->finalize_by_ceo,'cand_finalize_ro' =>$check_finalize->finalized_ac,'sechdul' => $sechdul,'seched'=>$seched, 'nomid'=>$nomid,'nomDetails'=>$nom,'persoanlDetails'=>$cand, 'ele_details'=>$ele_details,'all_state'=>$all_state,'all_dist'=>$all_dist,'all_ac'=>$all_ac,'pc'=>$pc]);	           
				}
				else{
					return redirect('/officer-login');
				}
  ...
  public function duplicate_drop(Request $request)
	...
	public function accepted_candidate(Request $request)
			...
			if(Auth::check()){
	         ...
	         if(isset($ele_details))
		    	...
		    	return view('admin.pc.ro.accepted-candidate',['user_data' => $d,'cand_finalize_ceo' =>$check_finalize->finalize_by_ceo,'cand_finalize_ro' =>$check_finalize->finalized_ac,'sechdul' => $sechdul,'sched'=>$seched,'lists'=>$list,'checkval'=>$val,'edetails'=>$ele_details]);
		         }
	        else {
	         	return Redirect::to('/officer-login');
	        	  }
    ...
    public function verifyac_wise_electors_details(Request $request){
            ...
            if ($validator->fails()) {
             ...
             return Redirect('/ropc/ac-wise-electors-details')->with('success', 'Electrol Data Updated Successfully !');             
