...
class AppointmentReportController extends Controller
{
    public $base = '';
    public $folder = '';
    public $view_path = "admin.nomination.onlinenom-report.eci_level.appointment.";
    public $view_path1 = "admin.nomination.onlinenom-report.ceo_level.appointment.";
    public $view_path2 = "admin.nomination.onlinenom-report.deo_level.appointment.";
    ...
    public function get_report(Request $request)
    {
        ...
        $data['heading_title']      = "Appointment Report";
        ...
        $action                     = Common::generate_url("online_nom/count-appointment-report-export");
        ...
        if(count($fetch_data)>0 && $fetch_data[0]['st_code'] != null){
            foreach($fetch_data as $each_data){
                ...
                $action_url = Common::generate_url('online_nom/dist-wise-appointment-report').'?'.implode('&', array_unique($request_array));
                $results[] = [
                    'sno'                               =>  $i_count++,
                    'st_code'                           =>  $each_data['st_code'],
                    'st_name'                           =>  getstatebystatecode($each_data['st_code'])->ST_NAME,
                    'action_url'                        =>  $action_url,
                    'pending_appointment'               =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'appointment'=> 'pend', 'date'=>$request->date)),
                    'done_appointment'                  =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'appointment'=> 'done', 'date'=>$request->date)),
                    'total_nomination'                  =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'appointment'=> '', 'date'=>$request->date)),
                    'pending_appointment_url'           =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['appointment'=>'appointment=pend']))),
                    'done_appointment_url'              =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['appointment'=>'appointment=done']))),
                    'total_nomination_url'              =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['appointment'=>'appointment='])))
                ];
            }
        }
        ...
        return view($this->view_path . 'onlinen_appointment_report', $data);
    }
    ...
    public function get_report_pdf(Request $request){
        ...
        $pdf = \PDF::loadView($this->view_path.'.onlinen_appointment_report_pdf',$data);
...
############################### For_dist_wise_report #######################################
    ...
    public function get_report_dist(Request $request)
    {
        ...
        $st_code                    = $request_filter['st_code'];
        $dist_no                    = $request_filter['dist_no'];
        $action                     = Common::generate_url("online_nom/dist-wise-appointment-report-export");
        ...
        if(Auth::user()->designation=='DEO'){
            ...
            $back_url = Common::generate_url("online_nom/count-appointment-report");
        }
        $data['buttons'][]  = [
            'name' => 'Back',
            'href' =>  $back_url,
        ...
        if(count($fetch_data)>0 && $fetch_data[0]['st_code'] != null){
            foreach($fetch_data as $each_data){
                ...
                $action_url = Common::generate_url('online_nom/ac-wise-appointment-report').'?'.implode('&', \array_unique($request_array));
                ...
                $results[] = [
                    ...
                    'action_url'                        =>  $action_url,
                    'pending_appointment'               =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'dist_no'=>$each_data['dist_no'], 'appointment'=> 'pend', 'date'=>$request->date)),
                    'done_appointment'                  =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'dist_no'=>$each_data['dist_no'],'appointment'=> 'done', 'date'=>$request->date)),
                    'total_nomination'                  =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'dist_no'=>$each_data['dist_no'],'appointment'=> '', 'date'=>$request->date)),
                    'pending_appointment_url'           =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['appointment'=>'appointment=pend']))),
                    'done_appointment_url'              =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['appointment'=>'appointment=done']))),
                    'total_nomination_url'              =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['appointment'=>'appointment='])))
                ];
        ...
        return view($this->view_path1 . 'onlinen_appointment_report_st', $data);
    }

    public function get_report_dist_pdf(Request $request){
        $data = $this->get_report_dist($request->merge(['is_export' => 1]));
        $name_pdf = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
        $pdf = \PDF::loadView($this->view_path1.'.onlinen_appointment_report_st_pdf',$data);
        return $pdf->download($name_pdf.'_'.date('d-m-Y').'_'.time().'.pdf');
    }
    ...
    public function get_report_ac(Request $request)
    {
        ...
        $st_code                    = $request_filter['st_code'];
        $dist_no                    = $request_filter['dist_no'];
        $action                     = Common::generate_url("online_nom/ac-wise-appointment-report-export");
        ...
        if(Auth::user()->designation=='DEO'){
            $back_url = Common::generate_url("online_nom/dist-wise-appointment-report").'?'.implode('&', $request_array);
        }else{
            $back_url = Common::generate_url("online_nom/dist-wise-appointment-report").'?'.implode('&', [$request_array[0]]);
        }
        $data['buttons'][]  = [
            'name' => 'Back',
            'href' =>  $back_url,
        ...
        if(count($fetch_data)>0 && $fetch_data[0]['st_code'] != null){
            foreach($fetch_data as $each_data){    
                $results[] = [
                    ...
                    'pending_appointment'               =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'dist_no'=>$each_data['dist_no'],'ac_no'=>$each_data['ac_no'], 'appointment'=> 'pend', 'date'=>$request->date)),
                    'done_appointment'                  =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'dist_no'=>$each_data['dist_no'],'ac_no'=>$each_data['ac_no'],'appointment'=> 'done', 'date'=>$request->date)),
                    'total_nomination'                  =>  OnlineNomModel::get_count_appointment_wise(array('st_code'=>$each_data['st_code'], 'dist_no'=>$each_data['dist_no'],'ac_no'=>$each_data['ac_no'],'appointment'=> '', 'date'=>$request->date)),
                    'pending_appointment_url'           =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['ac_no'=>'ac_no='.$each_data['ac_no'],'appointment'=>'appointment=pend']))),
                    'done_appointment_url'              =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['ac_no'=>'ac_no='.$each_data['ac_no'],'appointment'=>'appointment=done']))),
                    'total_nomination_url'              =>  Common::generate_url('online_nom/listofcandidate').'?'.implode('&', array_unique(array_merge($request_array, ['ac_no'=>'ac_no='.$each_data['ac_no'],'appointment'=>'appointment='])))
                ];
            }
        }
        $data['results'] = $results;
        ...
        return view($this->view_path2 . 'onlinen_appointment_report_dist', $data);
    }

    public function get_report_ac_pdf(Request $request){
        $data = $this->get_report_ac($request->merge(['is_export' => 1]));
        $name_pdf = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
        $pdf = \PDF::loadView($this->view_path2.'onlinen_appointment_report_dist_pdf',$data);
        return $pdf->download($name_pdf.'_'.date('d-m-Y').'_'.time().'.pdf');
    }
