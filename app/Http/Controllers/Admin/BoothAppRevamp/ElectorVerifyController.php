...
use App\commonModel;  
use App\models\Admin\BoothAppRevamp\{PollingStation, StateModel, AcModel, DistrictModel, ElectorVerifyModel,ElectorsVerificationByPsModel};
...
class ElectorVerifyController extends Controller {
  ...
  public $base          = 'ro';
  public $action_state  = 'eci/booth-app-revamp/elector-verify-report';
  public $action_ac     = 'eci/booth-app-revamp/elector-verify-report/state/ac';
  ...
  public function elector_verification_report(Request $request){
	...
	$this->action_state =Common::generate_url('booth-app-revamp/elector-verify-report');
	$this->action_ac =Common::generate_url('booth-app-revamp/elector-verify-report/state/ac');
    ...
    $title_array  = [];
    $data['heading_title']  = 'Booth App - Electors Verification Report';
   ...
   $statewise_results = [];
	...
	//dd($filter_election);
	  ...
	  $total_electors = ElectorsVerificationByPsModel::total_count_record($filter_election);
	  //dd($total_electors);
	  $po_pro = ElectorVerifyModel::elector_verify_report($filter_election);
	 // dd($po_pro);
  //  echo "<pre>";print_r($filter_election);die;
          ...
          $individual_filter                = implode('&', $individual_filter_array);
            ...
            //SETTING DATABASE RESULTS FOR 
              $statewise_results[] = [
                        ...
                        'total_verify'     => $po_pro_data['total_verify'],
						'total_unverify' => $po_pro_data['total_unverify'],
    ...
    if($request->has('is_excel')){
		...
		//form filters
		$data['filter_action'] = Common::generate_url("booth-app-revamp/elector-verify-report/state/ac");
	...
	return view($this->view.'.Reports.elector-verify-report', $data);

  }
  

   //officer-assignment-report excel function starts
    public function elector_verification_report_excel(Request $request){

    set_time_limit(6000);
    $data = $this->elector_verification_report($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];

    $export_data[] = ['State/UT Name','Total PS','Total Verified','Total Verified'];
    $headings[] = [];
  ...
  public function elector_verification_report_pdf(Request $request){
    set_time_limit(6000);
    $data = $this->elector_verification_report($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path.'.Reports.elector-verify-report-pdf',$data);
    return $pdf->download($name_excel.'_'.date('d-m-Y').'_'.time().'.pdf');
  }
  //officer-assignment-report pdf function ends
  
  //officer-assignment-report-ac function ends
  public function elector_verification_report_ac(Request $request){
  ...
  //GETTING LOGGED IN USERS DATA FROM AUTH 
   ...
   $statewise_results = [];
    ...
    //dd($filter_election);
      ...
      $po_pro   = ElectorVerifyModel::elector_verify_report_ac($filter_election);
          ...
          $individual_filter                = implode('&', $individual_filter_array);
            ...
            //SETTING DATABASE RESULTS FOR 
              $statewise_results[] = [
                        ...
                        'total_verify'     => $po_pro_data['total_verify'],
						'total_unverify'     => $po_pro_data['total_unverify'],
                        "href"         => Common::generate_url('booth-app-revamp/elector-verify-ps-wise-report/state/ac').'?st_code='.$state_result['st_code'].'&ac_no='.$po_pro_data['ac_no'].'&phase_no='.@$this->phase_no
                      ]; 
  ...
  return view($this->view.'.Reports.elector-verify-report-ac', $data);

  }
  //officer-assignment-report-ac function starts

   //officer-assignment-report-ac excel function starts
    public function elector_verification_report_ac_excel(Request $request){

    set_time_limit(6000);
    $data = $this->elector_verification_report_ac($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];

    $export_data[] = ['State/UT Name','CONST No','CONST Name','Total PS','Total Verified','Total Unverified'];
      $headings[]=[];
    foreach ($data['results'] as $lis) {
  ...
  public function elector_verification_report_ac_pdf(Request $request){
    set_time_limit(6000);
    $data = $this->elector_verification_report_ac($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path.'.Reports.elector-verify-report-ac-pdf',$data);
    return $pdf->download($name_excel.'_'.date('d-m-Y').'_'.time().'.pdf');
  }
  //officer-assignment-report-ac pdf function ends
  
  public function elector_verification_pswise_report(Request $request){
  ...
  //CHECKING FOR USER TYPE AND SETTING VARIABLES FOR IT STARTS
  $this->action_state =Common::generate_url('booth-app-revamp/elector-verify-ps-wise-report/state/ac');
  $this->action_ac =Common::generate_url('booth-app-revamp/elector-verify-ps-wise-report/state/ac');
  ...
  //GETTING LOGGED IN USERS DATA FROM AUTH 
   ...
   $statewise_results = [];
    ...
    //dd($filter_election);
      ...
      //PO AND PRO LOOP STARTS
      $po_pro   = ElectorVerifyModel::elector_verify_report_pswise($filter_election);
        //dd($po_pro);
         foreach ($po_pro as $po_pro_data) {
          ...
          $individual_filter                = implode('&', $individual_filter_array);
            ...
            //SETTING DATABASE RESULTS FOR 
              $statewise_results[] = [
                        ...
                        'const_no'     => $po_pro_data['ac_no'],
                        'const_name'   => $po_pro_data['ac_name'],
						'ps_name'   => $po_pro_data['PS_NO'].'-'.$po_pro_data['PS_NAME_EN'],
                        'verified'     => ($po_pro_data['is_verify']<>'' && $po_pro_data['is_verify']==1)?'Done':'Not Done',
                      ]; 
    ...
    //form filters
    $data['filter_action'] = Common::generate_url("booth-app-revamp/elector-verify-ps-wise-report/state/ac");
  ...
  return view($this->view.'.Reports.elector-verify-report-pswise', $data);

  }
  
  //officer-assignment-report-ac excel function starts
    public function elector_verification_pswise_report_excel(Request $request){

    set_time_limit(6000);
    $data = $this->elector_verification_pswise_report($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];

    $export_data[] = ['State/UT Name','CONST No','CONST Name','PS Name','Verification'];
      $headings[]=[];
    foreach ($data['results'] as $lis) {
  ...
  //officer-assignment-report-ac pdf function ends
  public function elector_verification_pswise_report_pdf(Request $request){
    set_time_limit(6000);
    $data = $this->elector_verification_pswise_report($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path.'.Reports.elector-verify-pswise-report-pdf',$data);
    return $pdf->download($name_excel.'_'.date('d-m-Y').'_'.time().'.pdf');
  }
  //officer-assignment-report-ac pdf function ends
