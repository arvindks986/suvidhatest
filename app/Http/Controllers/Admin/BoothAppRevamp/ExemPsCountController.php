...
use App\commonModel;  
use App\models\Admin\BoothAppRevamp\{PollingStation, StateModel, AcModel, DistrictModel, ExemPsCountModel,TblAnalyticsDashboardModel};
...
class ExemPsCountController extends Controller {
  ...
  public $base          = 'ro';
  public $action_state  = 'eci/booth-app-revamp/exemted-ps-count-report';
  public $action_ac     = 'eci/booth-app-revamp/exemted-ps-count-report/state/ac';
  ...
  public function __construct(Request $request){
	...
	public function exemted_ps_count_report(Request $request){
	...
	$this->action_state =Common::generate_url('booth-app-revamp/exemted-ps-count-report');
	$this->action_ac =Common::generate_url('booth-app-revamp/exemted-ps-count-report/state/ac');
    ...
    $title_array  = [];
    $data['heading_title']  = 'Booth App - Exempted PS Count Report';
   ...
   $statewise_results = [];
    ...
    $filter_election = [
	  ...
	  $exempted_ps = TblAnalyticsDashboardModel::total_exempted_ps($filter_election);
          ...
          $individual_filter                = implode('&', $individual_filter_array);
            ...
            //SETTING DATABASE RESULTS FOR 
              $statewise_results[] = [
                        ...
                        'total_exemps'     =>$exempted_ps_data['total_exempted'],
    ...
    if($request->has('is_excel')){
		...
		//form filters
		$data['filter_action'] = Common::generate_url("booth-app-revamp/exemted-ps-count-report");
	...
	return view($this->view.'.Reports.exempscount.exemted-ps-count-report', $data);

  }
  

   //officer-assignment-report excel function starts
    public function exemted_ps_count_report_excel(Request $request){

    set_time_limit(6000);
    $data = $this->exemted_ps_count_report($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];

    $export_data[] = ['State/UT Name','Total PS','Total Exempted PS'];
      $headings[]=[];
  ...
  public function exemted_ps_count_report_pdf(Request $request){
    set_time_limit(6000);
    $data = $this->exemted_ps_count_report($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path.'.Reports.exempscount.exemted-ps-count-report-pdf',$data);
    return $pdf->download($name_excel.'_'.date('d-m-Y').'_'.time().'.pdf');
  }
  //officer-assignment-report pdf function ends

  //officer-assignment-report-ac function ends
   public function exemted_ps_count_report_ac(Request $request){
  ...
  //GETTING LOGGED IN USERS DATA FROM AUTH 
   ...
   $statewise_results = [];
    ...
    //dd($filter_election);
      ...
      $po_pro   = TblAnalyticsDashboardModel::total_exempted_ps_ac($filter_election);
       // dd($state_result);
         foreach ($po_pro as $po_pro_data) {
			if($state_result['st_code'] == $po_pro_data->st_code){
            $state_name = '';
            //PS COUNT
            $filter_ac = [
              'phase_no' => $this->phase_no,
          ...
          $individual_filter                = implode('&', $individual_filter_array);
            ...
            //SETTING DATABASE RESULTS FOR 
              $statewise_results[] = [
                        ...
                        'total_ps'     => $po_pro_data['total_ps'],
                        'total_exempted'     => $po_pro_data['total_exempted'],
                        "href"         => Common::generate_url('booth-app-revamp/exemted-ps-count-pswise-report/state/ac').'?st_code='.$state_result['st_code'].'&ac_no='.$po_pro_data['ac_no'].'&phase_no='.@$this->phase_no
                      ]; 
    ...
    //form filters
    $data['filter_action'] = Common::generate_url("booth-app-revamp/exemted-ps-count-report/state/ac");
  ...
  return view($this->view.'.Reports.exempscount.exemted-ps-count-report-ac', $data);

  }
  //officer-assignment-report-ac function starts

   //officer-assignment-report-ac excel function starts
    public function exemted_ps_count_report_ac_excel(Request $request){

    set_time_limit(6000);
    $data = $this->exemted_ps_count_report_ac($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];

    $export_data[] = ['State/UT Name','CONST No','CONST Name','Total PS','Total Exempted PS'];
      $headings[]=[];
    foreach ($data['results'] as $lis) {
  ...
  public function exemted_ps_count_report_ac_pdf(Request $request){
    set_time_limit(6000);
    $data = $this->exemted_ps_count_report_ac($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path.'.Reports.exempscount.exemted-ps-count-report-ac-pdf',$data);
    return $pdf->download($name_excel.'_'.date('d-m-Y').'_'.time().'.pdf');
  }
  //officer-assignment-report-ac pdf function ends
  
  //Mapped location PS Wise
  public function exemted_ps_count_ps_wise(Request $request){
  ...
  //CHECKING FOR USER TYPE AND SETTING VARIABLES FOR IT STARTS
  $this->action_state =Common::generate_url('booth-app-revamp/exemted-ps-count-pswise-report');
  $this->action_ac =Common::generate_url('booth-app-revamp/exemted-ps-count-pswise-report/state/ac');
  ...
  //GETTING LOGGED IN USERS DATA FROM AUTH 
    ...
    //LISTING ALL STATES FOR DATABASE RESULTS
    $states = StateModel::get_states($filter);
   ...
   $statewise_results = [];
    ...
    $filter_election = [
      ...
      //PO AND PRO LOOP STARTS
	  $umps = TblAnalyticsDashboardModel::total_exempted_ps_pswise($filter_election);
//dd($umps);
         foreach ($umps as $ps_data) {
          if($state_result['st_code'] == $ps_data->st_code){
            $state_name = '';
            //PS COUNT
            $filter_ac = [
              'phase_no'=> $this->phase_no,
      ...
      //foreach ($po_pro as $po_pro_data) {
            ...
            //SETTING DATABASE RESULTS FOR 
              $statewise_results[] = [
						...
						'is_exempted'      => $ps_data['is_exempted']
                      ]; 
    ...
    //form filters
    $data['filter_action'] = Common::generate_url("booth-app-revamp/exemted-ps-count-pswise-report/state/ac");
	...
	return view($this->view.'.Reports.exempscount.exemted-ps-count-pswise-report', $data);  
  }
  //officer-assignment-report excel function starts
    public function exemted_ps_count_ps_wise_excel(Request $request){

    set_time_limit(6000);
    $data = $this->exemted_ps_count_ps_wise($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];

    $export_data[] = ['S No.','State/UT Name','Const No/Name','PS No/Name','Exempted'];
    $i=1;
    $headings[]=[];
    foreach ($data['results'] as $lis) {
  ...
  //officer-assignment-report pdf function ends
  public function exemted_ps_count_ps_wise_pdf(Request $request){
    set_time_limit(6000);
    $data = $this->exemted_ps_count_ps_wise($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',',': ',' '], ['_','-','_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path.'.Reports.exempscount.exemted-ps-count-pswise-report-pdf',$data);
    return $pdf->download($name_excel.'_'.date('d-m-Y').'_'.time().'.pdf');
  }
