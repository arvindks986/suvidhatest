...
winn.lead_total_vote,sum(cdac.gen_electors_male + cdac.gen_electors_female + cdac.gen_electors_other + cdac.service_male_electors + cdac.service_female_electors + cdac.service_third_electors + cdac.nri_male_electors + cdac.nri_female_electors + cdac.nri_third_electors) as electors_total ,m.st_name,
round((lead_total_vote/sum(cdac.gen_electors_male + cdac.gen_electors_female + cdac.gen_electors_other + cdac.service_male_electors + cdac.service_female_electors + cdac.service_third_electors + cdac.nri_male_electors + cdac.nri_female_electors + cdac.nri_third_electors) *100),0) AS CC
   ...
   //   $pdf = \App::make('dompdf.wrapper');
			...
			$pdf = PDF::loadView('IndexCardReports/StatisticalReports/Vol2/eciwinning-condidate-analysis-over-total-electors-pdf', compact('arrayData'));
                ...
                if(verifyreport(31)){
                  ...
                  $file_name = 'Winning Candidate Analysis Over Total Electors'.date('YmdHis').'.pdf';
                  ...
                  $pdf->save(public_path('uploads/statistical_report/31/'.$file_name));
...
return Excel::create('Winning Candidate Analysis Over Total Electors', function($excel) use ($arrayData) {
                    $excel->sheet('mySheet', function($sheet) use ($arrayData) {
						...
						$sheet->cells('D3', function($cells) {
                            $cells->setValue('Winner with >10% to <= 20%');
                            $cells->setFont(array('name' => 'Times New Roman', 'size' => 10, 'bold' => true));
                            $cells->setAlignment('center');
                        });
                        $sheet->cells('E3', function($cells) {
                            $cells->setValue('Winner with >20% to <=30%');
                            $cells->setFont(array('name' => 'Times New Roman', 'size' => 10, 'bold' => true));
                            $cells->setAlignment('center');
                        });
                        $sheet->cells('F3', function($cells) {
                            $cells->setValue('Winner with >30% to <=40%');
                            $cells->setFont(array('name' => 'Times New Roman', 'size' => 10, 'bold' => true));
                            $cells->setAlignment('center');
                        });
						...
						$sheet->cells('G3', function($cells) {
                            $cells->setValue('Winner with >40% to <=50%');
                            $cells->setFont(array('name' => 'Times New Roman', 'size' => 10, 'bold' => true));
                            $cells->setAlignment('center');
                        });
                        $sheet->cells('H3', function($cells) {
                            $cells->setValue('Winner with >50% to <=60%');
                            $cells->setFont(array('name' => 'Times New Roman', 'size' => 10, 'bold' => true));
                            $cells->setAlignment('center');
                        });
                        $sheet->cells('I3', function($cells) {
                            $cells->setValue('Winner with >60% to <=70%');
                            $cells->setFont(array('name' => 'Times New Roman', 'size' => 10, 'bold' => true));
                            $cells->setAlignment('center');
                        });
						$sheet->cells('J3', function($cells) {
                            $cells->setValue('Winner with > 70%');
                            $cells->setFont(array('name' => 'Times New Roman', 'size' => 10, 'bold' => true));
                            $cells->setAlignment('center');
                        });
                        $i = 4;
