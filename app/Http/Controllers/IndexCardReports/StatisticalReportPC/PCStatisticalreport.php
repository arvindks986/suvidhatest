       ...
       $user_data = $d;
        ...
        DB::enableQueryLog();
		 ...
		 $partywiseseatwons=DB::select("SELECT SUM(A.total_vote) AS totalvotebyparty ,B.* FROM counting_pcmaster A JOIN
				...
				WHERE TEMP.st_code=cpm.st_code AND cpm.`party_id` != '1180'
	...
	// return view('IndexCardReports.pcstatisticalreport.partywiseseatwonvalidvotes',compact('datanew','user_data'));
       ...
       $user_data = $d;
        ...
        $pdf = \App::make('dompdf.wrapper');
			...
			$pdf->loadView('IndexCardReports.pcstatisticalreport.pdfpartywiseseatwonvalidvotes', ['datanew' => $datanew]);

         if(verifyreport(18)){
        
                  $file_name = 'downloadpartywiseseatwonvalidvotes'.date('YmdHis').'.pdf';
                  ...
                  $pdf->save(public_path('uploads/statistical_report/18/'.$file_name));
                      ...
                      $sheet->setSize('B3', 15, 25);
                       ...
                       $sheet->cells('E3', function($cells) {
                        ...
                        foreach($partywiseseatwon['partdetails'] as $rowdata) {
                                     ...
                                     $sheet->cell('B' . $i, $partywiseseatwon['leadtypename']);
