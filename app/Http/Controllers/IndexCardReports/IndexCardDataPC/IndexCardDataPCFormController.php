...
class IndexCardDataPCFormController extends Controller
{
    ...
    public function pcwisedata(Request $request){
        ...
        if($user->designation == 'CEO'){
        	...
        	return view('IndexCardReports.IndexCardDataPCWise.indexcardreportpcselect', compact('session','pcList','user_data','sched'));
        }elseif($user->designation == 'DEO'){
            $pcList = DB::table('m_pc')
        				->select(['m_pc.PC_NO','PC_NAME'])
						->join('m_ac',[['m_ac.st_code','m_pc.st_code'],['m_ac.PC_NO','m_pc.PC_NO']])
        				->where(array(
							'm_pc.st_code' => $user->st_code,
							'DIST_NO_HDQTR' => $user->dist_no
						))
						->groupBy('m_pc.PC_NO')
						->orderBy('m_pc.PC_NO','ASC')
        				->get()->toArray();
	...
	public function pcwisebriefdata(Request $request){
		...
		//dd($pcList);
        	...
        	// $pcList = DB::table('m_pc')
        				...
        				// ->get()->toArray();
        	// echo "<pre>"; print_r($pcList); die;
        	return view('IndexCardReports.IndexCardDataPCWise.indexcardbriefreportpcselect', compact('session','pcList','user_data','sched'));
        }elseif($user->designation == 'DEO'){
            $pcList = DB::table('m_pc')
        				->select(['m_pc.PC_NO','PC_NAME'])
						->join('m_ac',[['m_ac.st_code','m_pc.st_code'],['m_ac.PC_NO','m_pc.PC_NO']])
	...
	//	print_r($request->all()); die;
      ...
      //dd($session);
    	...
    	$st_code = $request->st_code = $user->st_code;
		...
		//echo '<pre>'; print_r($getelectorsacwise); die;
		
	
			return view('IndexCardReports/IndexCardDataPCWise/indexcardreportpc',compact('getIndexCardDataCandidatesVotesACWise','session','getIndexCardDataPCWise','st_code','pc','getelectorsacwise','user_data', 'sched'));	
	...
	//	print_r($request->all()); die;
      ...
      //dd($session);
		...
		return view('IndexCardReports/IndexCardDataPCWise/indexcardbriefedreport',compact('getIndexCardDataCandidatesVotesACWise','session','getIndexCardDataPCWise','st_code','pc','getelectorsacwise','user_data', 'sched'));
...
public function getindexcarddatapdf(Request $request, $pc = 0){
...
public function getindexcarddataexcel(Request $request, $pc = 0){
...
public function getindexcardbriefdatapdf(Request $request, $pc = 0){
...
m_ac.ST_CODE = '$st_code' and m_ac.PC_NO = $pc ORDER BY m_district.DIST_NAME ASC");
    ...
    public function feedbackForm(Request $request){
      ...
      $resultPCs = DB::table('m_pc')
                    ...
                    ->where('st_code',$user->st_code)
                    ->get()->toArray();
      // echo "<pre>"; print_r($resultPCs); die;
      return view('IndexCardReports.IndexCardDataPCWise.feedback', compact('user_data','sched','resultACs','resultPCs'));
    }
    public function feedbackSubmit(Request $request){
      ...
      if($request->selectChange == 'AC'){
        foreach ($request->acno as $value) {
          $insertData[] = array(
            'for_ac_no'               => $value,
            'st_code'                 => $user_data->st_code,
            'request_start_datetime'  => date('Y-m-d H:i:s', strtotime(Input::get('rStartDateAC'))),
            'request_end_datetime'    => date('Y-m-d H:i:s', strtotime(Input::get('rEndDateAC'))),
            'request_field'           => $request->rFieldAC,
            'request_description'     => $request->rFieldDescAC,
            'requestor_id'            => $user_data->officername
          );
        }
      }else{
        foreach ($request->pcno as $value) {
          $insertData[] = array(
            'for_pc_no'               => $value,
            'st_code'                 => $user_data->st_code,
            'request_start_datetime'  => date('Y-m-d H:i:s', strtotime(Input::get('rStartDatePC'))),
            'request_end_datetime'    => date('Y-m-d H:i:s', strtotime(Input::get('rEndDatePC'))),
            'request_field'           => $request->rFieldPC,
            'request_description'     => $request->rFieldDescPC,
            'requestor_id'            => $user_data->officername
          );
        }
      }
      foreach ($insertData as $key => $value) {
        $insertId = DB::table('feedback_request_ic')->insertGetId($value);
      }
      if($insertId){
        /*$message = "Hello $user_data->officername, Your request for data change has been successfully generated";
        $path = 'http://enterprise.smsgupshup.com/GatewayAPI/rest';
        $data_array = array(
                "method"        => "sendMessage",
                "send_to"       => $user_data->Phone_no,
                "msg"           => $message,
                "msg_type"      => "TEXT",
                "userid"        => "2000184184",
                "auth_scheme"   => 'PLAIN',
        ...
        $make_call = $this->callAPI('GET', $path, $data_array);*/
        return redirect()->action('IndexCardReports\IndexCardDataPC\IndexCardDataPCFormController@feedbackForm')->with('success','Request has been generated successfully! SMS has been sent to your registered mobile number.');
      }else{
...
public function myRequest(Request $request){
	  ...
	  $dt = Carbon::now();
		...
		->select('feedback_request_ic.*','m_ac.ac_name','m_pc.PC_NAME')
		...
		->where('requestor_id',$user->officername)
      ...
      return view('IndexCardReports.IndexCardDataPCWise.myRequest', compact('user_data','data'));
    }
	...
	public function finalizerequestsubmit(Request $request){
      ...
      return Redirect::to('pcceo/myrequestindexcard');
    }
