...
where ccil.st_code=TEMP.ST_CODE AND ccil.pc_no=TEMP.pc_no  group by TEMP.pc_no )as total_votes,
...
WHERE TEMP.PC_NO=cdac.pc_no and TEMP.ST_CODE=cdac.st_code  group by TEMP.PC_NO )AS total_electors
FROM
(
select mm.ST_CODE,mm.ST_NAME as st_name,mp.PC_NO,mp.PC_NAME,cpd.cand_name,if(cpd.cand_gender='M','',cpd.cand_gender) AS cand_gender,
cpd.cand_age, cpd.cand_category, cci.party_abbre,
ms.SYMBOL_DES,(cci.evm_vote+cci.migrate_votes) as general,
cci.postal_vote as postal,
cci.total_vote as cand_total_vote,
(select new_srno from candidate_nomination_detail cnd2 where cnd2.st_code=cci.st_code AND cnd2.pc_no=cci.pc_no
and cnd2.application_status='6' and cnd2.finalaccepted=1 and cnd2.candidate_id = cci.candidate_id)as new_srno
...
ORDER BY mp.ST_CODE,mp.PC_NO,new_srno ASC
)TEMP");


$all_india_Data = DB::select("SELECT sum(evm_vote+migrate_votes) as all_india_evm,sum(`postal_vote`) as all_india_postal,sum(`total_vote`) as all_india_total FROM `counting_pcmaster`");
				...
				foreach($candidateData as $raw){
					...
					$dataArr[$raw->st_name][$raw->PC_NO.' . '.$raw->PC_NAME.' <b> ( Total Electors </b>'.$raw->total_electors.')'][] = array(
			...
			$pdf = PDF::loadView('IndexCardReports.StatisticalReports.Vol1.constituency-wise-detailed-result-pdf',compact('dataArr','user_data','all_india_Data'));
                  ...
                  $pdf->save(public_path('uploads/statistical_report/33/'.$file_name));
            ...
            $sheet->cell('L2', function($cells) {
               $cells->setValue('% of votes secured');
                ...
                $cells->setFont(array('name' => 'Times New Roman', 'size' => 12, 'bold' => true));
					...
					$sheet->setSize('A3', 20,50);
					$sheet->setSize('B3', 20,50);
					$sheet->setSize('C3', 50,50);
					$sheet->setSize('D3', 10,50);
					$sheet->setSize('E3', 10,50);
					$sheet->setSize('F3', 15,50);
					$sheet->setSize('G3', 15,50);
					$sheet->setSize('H3', 25,50);
					$sheet->setSize('I3', 10,50);
					$sheet->setSize('J3', 10,50);
					$sheet->setSize('K3', 10,50);
					$sheet->setSize('L3', 20,50);
					$sheet->setSize('M3', 20,50);
					$sheet->setSize('N3', 15,50);
				   ...
				   $sheet->cell("C3", function($cells) {
					...
					$sheet->cell("M3", function($cells) {
						$cells->setValue('OVER TOTAL VOTES POLLED IN CONSTITUENCY');
					});
			...
			foreach($dataArr as $key => $data){
				...
				foreach($data as $key1 => $raw){
					...
					foreach($raw as $row){
                        ...
                        $sheet->cell('C'.$i, $row['cand_name']); 
                        $sheet->cell('D'.$i, strtoupper($row['cand_gender']));
                        $sheet->cell('E'.$i, $row['cand_age']); 
                        $sheet->cell('F'.$i, strtoupper($row['cand_category'])); 
                        $sheet->cell('G'.$i, $row['party_abbre']); 
                        $sheet->cell('H'.$i, $row['SYMBOL_DES']); 
                        $sheet->cell('I'.$i, ($row['general_vote']) ? $row['general_vote'] : '=(0)'); 
                        $sheet->cell('J'.$i, ($row['postal_vote']) ? $row['postal_vote'] : '=(0)'); 
                        $sheet->cell('K'.$i, ($row['cand_total_vote']) ? $row['cand_total_vote'] : '=(0)'); 
                        $sheet->cell('L'.$i, $totalelectorPercent);  
                        $sheet->cell('M'.$i, $totalvotespolled);
						$sheet->cell('N'.$i, $row['total_electors']);
                          $i++; 
                      } 
