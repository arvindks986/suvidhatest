...
namespace App\Http\Controllers\IndexCardReports\IndexCardDataEci;
...
class IndexCardEciController extends Controller
{
    ...
    public function pcwisedata(Request $request){
		$dt = Carbon::now();
        ...
        if(($user->role_id == '7') || ($user->role_id == '27')){
        	...
        	//echo "<pre>"; print_r($elections); die;
        	return view('IndexCardReports.IndexCardDataEci.indexcardreportpcselect', compact('session','stateList','user_data','sched'));
        }
    }
	...
	public function ajaxpccall(Request $request){
			  ...
			  $pcList = DB::table('m_pc')
						...
						->where('m_election_details.election_status','1')
						//->where('m_election_details.election_id','1')
        				->where('m_pc.ST_CODE',$request->st_code)
			...
			<?php
				foreach ($pcList as $pcLists) {
					?>
			<option value="<?php echo $pcLists->PC_NO; ?>"><?php echo $pcLists->PC_NO.'-'.$pcLists->PC_NAME; ?></option>
			<?php
				}
	...
	public function pcwisebriefdata(Request $request){
        ...
        if($user->role_id == '7' || $user->role_id == '27'){
			...
			$stateList = DB::table('m_state')
						...
						->groupBy('m_state.ST_CODE')
        				->get()->toArray();
        	return view('IndexCardReports.IndexCardDataEci.indexcardreportbreifselect', compact('session','stateList','user_data','sched'));
        }
    }
	...
	//	print_r($request->all()); die;
      ...
      //dd($session);
        $user = Auth::user();
           ...
           $session['election_detail'] = array();
           $session['election_detail']['st_code'] = $request->st_code;
		...
		if($request->st_code && $request->pc ){
		$getIndexCardDataPCWise = $this->getIndexCardDataPCWise($request->st_code, $request->pc, $election_detail);

    	$getIndexCardDataCandidatesVotesACWise = $this->getIndexCardDataCandidatesVotesACWise($request->st_code, $request->pc);
    	$getelectorsacwise = $this->getelectorsacwise($request->st_code, $request->pc);
		...
		$getIndexCardDataPCWise = 	$getIndexCardDataCandidatesVotesACWise = $getelectorsacwise = $pcList = array();
		}


			return view('IndexCardReports/IndexCardDataPCWise/indexcardreportpc',compact('getIndexCardDataCandidatesVotesACWise','session','getIndexCardDataPCWise','st_code','pc','getelectorsacwise','user_data', 'sched','stateList','pcList'));

    }



	 public function getindexcardbriefdata(Request $request){
        $user = Auth::user();
            $uid=$user->id;
            $d=$this->commonModel->getunewserbyuserid($user->id);
            $d=$this->commonModel->getunewserbyuserid($uid);
		...
		if($request->st_code && $request->pc ){
			$getIndexCardDataCandidatesVotesACWise = $this->getIndexCardDataCandidatesVotesACWise($request->st_code, $request->pc);   
			$getIndexCardDataPCWise = $this->getIndexCardDataPCWise($request->st_code, $request->pc, $election_detail = 1);
			$pcList = DB::table('m_pc')
        				->select(['m_pc.PC_NO','m_pc.PC_NAME'])
						->join('m_election_details', function($join){
							$join->on('m_pc.ST_CODE','m_election_details.ST_CODE')
			...
			$getIndexCardDataPCWise = 	$getIndexCardDataCandidatesVotesACWise = $pcList = array();
		}

		//echo Route::get('pcceo/indexcardpc'); die;
		return view('IndexCardReports/IndexCardDataPCWise/indexcardbriefedreport',compact('getIndexCardDataCandidatesVotesACWise','getIndexCardDataPCWise','st_code','pc','user_data','stateList','pcList'));
    }


public function getindexcarddatapdf(Request $request, $st_code = 0, $pc = 0){
	...
	//	print_r($request->all()); die;
      ...
      //dd($session);
        $user = Auth::user();
           ...
           $session['election_detail'] = array();
           $session['election_detail']['st_code'] = $st_code;
          // $session['election_detail']['st_name'] = $user->placename;

    	$st_code = $request->st_code = $request->st_code;
		...
		$label = [
			'count' => 'SL. No.',
			'cand_name' => 'Name of Contesting Candidates <br>(in Block letters)',
			'cand_gender' => 'Sex <br> (Male/Female/Third Gender)',
			'cand_age' => 'Age <br> (Years)',
			'cand_category' => 'Category <br>(Gen/SC/ST)',
			'partyname' => 'Full Name of the Party',
			'party_symbol' => 'Election Symbol Allotted'
		];
		
		foreach($getIndexCardDataCandidatesVotesACWise['allACList'] as $key => $value){
			$label['pc_'.$key] = $key.'-'.$value;
		}
		if(in_array($pc,[1,2,3]) && $st_code == 'S09'){
			$label["migrate_votes"] 		= "Migrant Votes";
		}
		$label["valid_postal_votes"] 	= "Postal Votes";
		$label["total_valid_vote"] 		= "Total Valid Vote";
		
		foreach($getIndexCardDataCandidatesVotesACWise['candidatedataarray'] as $key => $result_ite){
			foreach($result_ite as $temp_ite){	
				$extra_data = [
					"count" 		=> ++$count,
					"cand_name" 	=> $temp_ite['cand_name'],
					"cand_gender" 	=> $temp_ite['cand_gender'],
					"cand_age" 		=> $temp_ite['cand_age'],
					"cand_category" => $temp_ite['cand_category'],
					"partyname" 			=> $temp_ite['partyname'],
					"party_symbol" 			=> $temp_ite['party_symbol']
				];
				$total['count'] = NULL;
				$total['cand_name'] = NULL;
				$total['cand_gender'] = NULL;
				$total['cand_age'] = NULL;
				$total['cand_category'] = NULL;
				$total['partyname'] = NULL;
				$total['party_symbol'] = NULL;
				
				foreach($temp_ite['acdata'] as $key => $acdata){
					$extra_data['pc_'.$key] = $acdata['vote_count'];
					if(isset($total['pc_'.$key])){
						$total['pc_'.$key] = $total['pc_'.$key]+$acdata['vote_count'];
					}else{
						$total['pc_'.$key] = $acdata['vote_count'];
					}
				}
				if(in_array($pc,[1,2,3]) && $st_code == 'S09'){
					$extra_data["migrate_votes"] 	= $temp_ite['migrate_votes'];
					$total["migrate_votes"] 		= NULL;
				}				
				
				//insert after pc login
				$extra_data["valid_postal_votes"] 	= $temp_ite['valid_postal_votes'];
				$extra_data["total_valid_votes"] 	= $temp_ite['total_valid_votes'];
				$total['valid_postal_votes'] = NULL;
				$total['total_valid_vote'] = NULL;
		...
		$data[] = $total; */
			...
			$pdf=PDF::loadView
		('IndexCardReports.IndexCardDataPCWise.indexcardreportpcpdf',compact('getIndexCardDataCandidatesVotesACWise','session','getIndexCardDataPCWise','st_code','pc','getelectorsacwise','user_data', 'sched','ele_details'));
...
public function getindexcarddataexcel(Request $request, $st_code = 0, $pc = 0){
          ...
          $user_data = $d;
			...
			return Excel::download(new IndexCardReport($getIndexCardDataCandidatesVotesACWise,$getIndexCardDataPCWise,$getelectorsacwise,$st_code,$pc), 'IndexCard Report.xlsx');


    }


public function getindexcardbriefdatapdf(Request $request, $st_code=0, $pc = 0){
...
//echo $pc;
    	...
    	$getIndexCardDataCandidatesVotesACWise = $this->getIndexCardDataCandidatesVotesACWise($st_code,$pc);
    	// echo "<pre>"; print_r($getIndexCardDataCandidatesVotesACWise); die;

    	$getIndexCardDataPCWise = $this->getIndexCardDataPCWise($st_code, $pc, $election_detail);
    	////Praveen Code/////////
    	//$getelectorsacwise = $this->getelectorsacwise($request->st_code, $request->pc);


			$pdf=PDF::loadView
		('IndexCardReports.IndexCardDataPCWise.indexcardbriefedreportpdf',compact('getIndexCardDataCandidatesVotesACWise','session','getIndexCardDataPCWise','st_code','pc','user_data', 'sched','ele_details'));
...
//echo '<pre>'; print_r($data_pc_wise); die;
    ...
    public function electiondataAcwise(Request $request){
       $data = $request->all();
       $st_code = $data['st_code'];
       $pc_no = $data['pc_no'];
       $ScheduleID = $data['ScheduleID'];
       $loggedinUserid = Auth::user()->id;
       //echo "<pre>"; print_r($data['electorsac']); die;
       foreach ($data['electorsac'] as $key => $value) {
           $dataForInsert = array(
               ...
               'e_ser_f'     =>  $value['ser_f'],
               'e_ser_t'     =>  $value['ser_t'],
               'e_total_m'   =>  $value['tot_m'],
               'e_total_f'   =>  $value['tot_f'],
               'e_total_o'   =>  $value['tot_o'],
                'e_total_all' => $value['tot_all'],
                'update_by'   =>  $loggedinUserid
           );
           DB::table('electors_detail_ac_ic')->insert($dataForInsert);
       }
       return redirect()->action('IndexCardReports\IndexCardDataPC\IndexCardDataPCFormController@getindexcarddata',['pc'=>$pc_no,'st_code'=>$st_code,'ScheduleID'=>$ScheduleID])->with('success','Data has been updated Successfully');
       }
    /**************************************************************/
    public function getAllPC(Request $request){
    	// $data = $request->all();
    	// echo "<pre>"; print_r($data['stcode']); die;
    	// DB::enableQueryLog();
    	$pcList = DB::table('m_pc')
    				->select(['PC_NO','PC_NAME'])
    				->where('ST_CODE',$request['stcode'])
    				->get()->toArray();
    	$options = '<option value="">Select PC</option>';
    	foreach ($pcList as $value) {
    		$options .= '<option value="'.$value->PC_NO.'">'.$value->PC_NAME.'</option>';
    	}
    	// $queue = DB::getQueryLog();
    ...
    public function getIndexCardDataCandidatesVotesACWise($st_code, $pc){
		...
		//echo '<pre>';print_r($responseFromIC); die;					
