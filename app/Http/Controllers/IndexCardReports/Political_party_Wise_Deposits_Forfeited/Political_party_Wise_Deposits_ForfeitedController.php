...
class Political_party_Wise_Deposits_ForfeitedController extends Controller
{
  ...
  //  dd($session);
    ...
    $sSelect = array(
     ...
     $datacandidate = DB::table('candidate_nomination_detail AS A')
                        ->select('A.cand_party_type as PARTYTYPE','A.st_code',
                         ...
                         ->WHERE('A.party_id', '!=', '1180')
                          ...
                          ->groupby('A.st_code','A.cand_party_type')
                         ->get()->toArray();

    //echo '<pre>';  print_r($datacandidate); die;

    $dataelectedcandidate = DB::table('winning_leading_candidate AS wlc')
                            ->select('wlc.st_code','A.cand_party_type as lead_party_type',
                              DB::raw('COUNT(wlc.leading_id) AS totalelected')
                                    )
							->join('candidate_nomination_detail AS A',[['A.candidate_id','wlc.candidate_id']])
							->WHERE('A.application_status', '6')
							->WHERE('A.finalaccepted', '1')
                              ->groupby('A.st_code','A.cand_party_type')
                              //->WHERE('status', '1')
   ...
   // echo '<pre>';  print_r($dataelectedcandidate); die;
    ...
    foreach ($totalstate as  $value) {
    	...
    	$datafdcandidate = DB::select("SELECT cp.st_code,
           ...
           where cp1.pc_no = cp.pc_no and cp.st_code =cp1.st_code and  C.cand_party_type = 'N' GROUP BY cp1.`pc_no` ),4) < .1666 THEN 1 ELSE 0 END) as fdN,

           SUM(CASE WHEN ROUND(cp.total_vote/(SELECT SUM(cp1.`total_vote`) as pctotalvotes FROM `counting_pcmaster` as cp1
           where cp1.pc_no = cp.pc_no and cp.st_code =cp1.st_code and  C.cand_party_type = 'S' GROUP BY cp1.`pc_no` ),4) < .1666 THEN 1 ELSE 0 END) as fdS,

           SUM(CASE WHEN ROUND(cp.total_vote/(SELECT SUM(cp1.`total_vote`) as pctotalvotes FROM `counting_pcmaster` as cp1
           where cp1.pc_no = cp.pc_no and cp.st_code =cp1.st_code and  C.cand_party_type = 'U' GROUP BY cp1.`pc_no` ),4) < .1666 THEN 1 ELSE 0 END) as fdU,

           SUM(CASE WHEN ROUND(cp.total_vote/(SELECT SUM(cp1.`total_vote`) as pctotalvotes FROM `counting_pcmaster` as cp1
           where cp1.pc_no = cp.pc_no and cp.st_code =cp1.st_code and  C.cand_party_type = 'Z' GROUP BY cp1.`pc_no` ),4) < .1666 THEN 1 ELSE 0 END) as fdZ,
           ...
           join candidate_nomination_detail AS C on C.candidate_id = cp.candidate_id
           ...
           AND cp.party_id != 1180 and C.application_status = '6' and C.finalaccepted = '1' AND cp.st_code = '$value->ST_CODE'");
      ...
      // dd($result);
       ...
       if($request->path() == "$prefix/Political_party_Wise_Deposits_Forfeited"){
        ...
        $pdf = \App::make('dompdf.wrapper');
			...
			$pdf->loadView('IndexCardReports.PoliticalPartyWiseDepositsForfeited.PoliticalpartyWiseDepositsForfeitedPDF',[
         ...
         if(verifyreport(19)){
                  ...
                  $file_name = 'Political Party Wise Deposit Forfeited'.date('YmdHis').'.pdf';
                  ...
                  $pdf->save(public_path('uploads/statistical_report/19/'.$file_name));
        ...
        return $pdf->download('Political Party Wise Deposit Forfeited.pdf');
        }
  ...
  return Excel::create('Political Party Wise Deposit Forfeited', function($excel) use ($statewisedata) {
