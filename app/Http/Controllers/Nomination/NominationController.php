<?php 
namespace App\Http\Controllers\Nomination;
...
class NominationController extends Controller
{
    ...
    if($request->old('pan_number')){
	  ...
	  $pannnn = "";
	  for( $pp = 0; $pp < strlen($oldPan); $pp++ ) {
		$b = ord($oldPan[$pp]);
		$a = $b ^ 123;  
		$pannnn .= chr($a);
	  }			
      $data['pan_number']  = $pannnn; 
    ...
    $data['acs'] = [];
   // $acs = PcModel::orderByRaw('ST_CODE, PC_NO ASC')->get();
    $acs = AcModel::get();
    //dd($acs);
    // foreach ($acs as $key => $ac_iterage) {
    //   $data['acs'][] = [
    //     'ac_no'         => $ac_iterage->PC_NO,
    //     'ac_name'       => $ac_iterage->PC_NO.'-'.$ac_iterage->PC_NAME,
    ...
    DB::commit();
     if(config('public_config.nomination_log'))
           {
           	$candid=Auth::id();
          $message['LogDescription']= 'Candidate Personal Details CandidateID: '.$candid;
...
//	echo "<pre>"; print_r($states); die("data");
	...
	//$sql = "select m_ac.AC_NO as ac_no, m_ac.ST_CODE as st_code, m_ac.DIST_NO_HDQTR as district_no, CONCAT(AC_NAME) as ac_name, ELECTION_ID as election_id, ELECTION_TYPEID as election_type from `m_ac` inner join `m_election_details` on (`m_election_details`.`ST_CODE` = `m_ac`.`ST_CODE` and `m_election_details`.`CONST_NO` = `m_ac`.`AC_NO`) where `election_status` = 1 and `m_election_details`.`CONST_TYPE` = 'PC' group by `m_ac`.`AC_NO`, `m_ac`.`ST_CODE` order by m_ac.ST_CODE,m_ac.AC_NO ASC";
   ...
   // $acs = AcModel::get_acs();
    ...
    foreach ($acs as $key => $ac_iterage) { 
      $data['acs'][] = [
        ...
        'election_type' => $ac_iterage->election_type,
        //'encoded'    => base64_encode($ac_iterage->pc_no),
      ];
    }
...
//
   ...
   if(empty($check)){ 
    ...
    try{ 
	 ...
	 // echo Session::get('nomination_id'); die;
      ...
      if(!$request->has('nomination_id')){
		...
		$destination_path = FileModel::get_file_path('uploads1/qrcode/'.$year.'/pc/'.$election_name.'/'.$st_code.'/'.$ac_no).'/'.$data['id'].'.png';
      ...
      DB::rollback();
       if(config('public_config.nomination_log'))
                 {
		          $message['LogDescription']= 'Error:'. $e->getMessage()  ;
    ...
    if(config('public_config.nomination_log'))
                 {
		          $message['LogDescription']= 'State: '.$st_code.' Const_no '.$ac_no. ' Election Name: '.$election_name  ;
		          $message['TransectionStatus']= 'SUCCESS';
...
//dd($acs);
    ...
    $data['resident_acs'] = [];
    $acs = PcModel::where('ST_CODE', $users['state'])->get();
...
//dd($data['resident_acs']);
  ...
  public function save_step_3(NominationPart12Request $request){	
    ...
    try{  
	 ...
	 if($request->recognized_party == '2'){ 
      ...
      if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'part I/II Form Error: '.$e;
          $message['TransectionStatus']= 'FAILED';
    ...
    DB::commit();
      if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'part I/II Form  Nomination Num: '.$get_nominattion_detail['nomination_no'];
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function save_step_4(NominationPart3Request $request){
	...
	$user=Auth::user();
  $message=array();
   $message['MobNo']= $user->mobile ?? '';
    $message['applicationType']= 'WebApp';
    $message['Module']= 'SUVIDHA';
    $message['TransectionType']= 'Online Nomination';
    $message['TransectionAction']= 'Nomination Part III';
	  
	    $get_nominattion_detail = NominationApplicationModel::get_nomination_application($input['nomination_id']);  
    ...
    try{  
      ...
      DB::rollback();
       ...
       if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'part III Form '.$e;
          $message['TransectionStatus']= 'FAILED';
    ...
    if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'part III Form  Nomination Num: '.$get_nominattion_detail['nomination_no'];
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function save_step_5(NominationPart3aRequest $request){
    ...
    $message['Module']= 'SUVIDHA';
    $message['TransectionType']= 'Online Nomination';
    $message['TransectionAction']= 'Nomination Part IIIA Form';
     $get_nominattion_detail = NominationApplicationModel::get_nomination_application($request->nomination_id); 
	...
	//die;	
      ...
      DB::rollback();
       ...
       if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Part IIIA Form Error: '.$e;
          $message['TransectionStatus']= 'FAILED';
    ...
    DB::commit();
     if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Part IIIA Form  Nomination Num: '.$get_nominattion_detail['nomination_no'];
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function save_nomination_finalize($id = 0, Request $request){
   ...
   $user=Auth::user();		
        ...
        $message['MobNo']= $user->mobile ?? '';
		    ...
		    $message['Module']= 'SUVIDHA';
		    $message['TransectionType']= 'Online Nomination';
        $message['TransectionAction']= 'Save Final Nomination ';
        
	    $nomination_num=$get_nominattion_detail['nomination_no'];
	...
	$this->sendSMS($mob[0]->mobile, $message); */
		...
		if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Nomination Final  NominationID: '.$nomination_num;
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function download_scheduled(Request $Request){
	...
	if(count($nom) > 0 ){
		...
		$data['ACname'] = $this->getAcName($nom[0]->st_code, $nom[0]->pc_no);
  ...
  public function getNominationBookedDetails($nomid){ 
   ...
   $nom=DB::connection('mysql')->table('nomination_application')
	...
	if(count($nom) > 0 ){ 
		...
		//////////////////
		return  $nom[0]->nomination_no.'***'.$nom[0]->name.'***'.$nom[0]->pc_no.'***'.$this->getAcName($nom[0]->st_code, $nom[0]->pc_no).'***'.$st.'***'.$nom[0]->updated_at.'***'.url('nomination/detail/'.encrypt_String($_REQUEST['id'])).'***'.url('nomination/download/'.encrypt_String($_REQUEST['id'])).'***'.$roname.'***'.$placename.' '.$ro_address_l1.'***'.$ro_address_l2.'***'.$this->getDist($nom[0]->st_code, $getDistNo).'***'.$this->getElection($nom[0]->election_type_id).'***'.$this->getState($nom[0]->st_code).'***'.$pty.'***'.$nom[0]->st_code.'***'.$cat;
	} else {
		return "NA";
	}
  ...
  //dd("sdsssss");
    ...
    foreach($results as $result){ 
      ...
      $data['results'][] = [
        ...
        'id'      => $result['id'],
        'name'    => $result['name'],
        'ac_name' => $result['pc_no'].'-'.$result['pc_name'],
        'election_name' => $result['election_name'],
        'status'        => $status,
        'updated_at'    =>$result['updated_at'],
        'step'       	=>$result['step'],
	...
	$chechnomDate=0;
	$chechnomDate = $this->getdateNom($nom[0]->st_code, $nom[0]->ac_no);
	if($chechnomDate==1){
	  return Redirect::back();
	}
	...
	$dist_code='';
	$dist_code  = $this->getDistNo($nom[0]->st_code, $nom[0]->ac_no);
	if(!empty($dist_code)){
	...
	$ac_save='';
	if(!empty($nom[0]->ac_no)){
	  $data['ac_save']=trim($nom[0]->ac_no);
	}
	...
	if(count($payment_config) > 0){
		...
		//Condition will be increase as payment gateway will be increase
			if($nom[0]->st_code=='S25'){//West Bengol
						...
						$data['amount']=trim($amt);
			
			} else if($nom[0]->st_code=='U07'){//Puduchery
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle';
			...
			} else if($nom[0]->st_code=='S11'){//Kerla
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle';
						...
						->table('payment_kerla_ro_mapping')
						->select('*')
						->where('ac_no', '=', $nom[0]->ac_no)
						->get()
						->toArray();
						if(count($payment_office_code)>0){
							$data['office_code'] = $payment_office_code[0]->office_code;
						}else{
							$data['office_code'] = '';
						}
				
			} else if($nom[0]->st_code=='S22'){//Tamilnadu
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-tm';
						...
						$nom_dist_no = "";
						$get_dist_byac = DB::connection('mysql')->table('m_ac')->select('DIST_NO_HDQTR')->where('AC_NO', '=', $nom[0]->ac_no)->first();
						...
						if(!empty($nom_dist_no)){
							...
							->select('*')
							->where('dist_no', '=', $nom_dist_no)
							->get()
							->toArray();
						}
						
						$data["nom_dist"] = $nom_dist_no;

						if(count($payment_office_code)>0){
							$data['district'] = $payment_office_code[0]->dist_name;
							$data['ddoCode'] = $payment_office_code[0]->ddo;
							$data['ddoOffice'] = $payment_office_code[0]->hod;
						}else{
							$data['district'] = '';
							$data['ddoCode'] = '';
							$data['ddoOffice'] = '';
			...
			} else if($nom[0]->st_code=='S03'){//Assam
						$data['isPaymentConfig']='YES';		
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle';
						...
						$data['DU']=trim('https://demo.eci.nic.in/suvidhaac/public/payment-return-first-call'); //Demo
						$data['RU']=trim('https://demo.eci.nic.in/suvidhaac/public/payment-return-first-call'); //Demo	
			...
			} else if($nom[0]->st_code=='S14'){ //Manipur
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-man';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
						...
						$payment_office_code =  DB::connection('mysql')
						->table('payment_manipur_ro_mapping')
						->select('*')
						->where('ac_no', '=', $nom[0]->ac_no)
						->get()
						->toArray();
						if(count($payment_office_code)>0){
							$data['office_code'] = $payment_office_code[0]->office_code;
							$data['treasury_code'] = $payment_office_code[0]->treasury_code;
						}else{
							$data['office_code'] = '';
							$data['treasury_code'] = '';
						}
			} else if($nom[0]->st_code=='S28'){ // Uttrakhand
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-uk';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
			...
			} else if($nom[0]->st_code=='S24'){ // Uttar Pradesh
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-up';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
			...
			} else if($nom[0]->st_code=='S05'){ // Goa
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-goa';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
			...
			} else if($nom[0]->st_code=='S19'){ // Punjab
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999);
						$rurl =  url('/').'/payment-return-handle-pun';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
						...
						$dist_data = DB::table('punjab_payment_gateway_district')
						->select('pg_dist_code', 'ddo_code', 'pay_loc_code')
						->where('DIST_NO', $dist_code)	
						->first();
						
						$dist_data2 = DB::table('payment_punjab_ro_mapping')
						->where('ac_no', $nom[0]->ac_no)	
						->first();
						
						$data['ddo_code'] = $dist_data->ddo_code;
						$data['pay_loc_code'] = $dist_data->pay_loc_code;						
						$data['tehsil_lgdid'] = $dist_data2->tehsil_lgdid;
						$data['district_lgdid'] = $dist_data2->district_lgdid;
						
						
						$data['dist_code'] = $dist_code;
						
						
			} else if($nom[0]->st_code=='S08'){ // Himachal Pradesh
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-hp';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
						...
						$payment_office_code =  DB::table('payment_hp_mapping')
						->select('*')
						->where('dist_no', $dist_code)
						->get()->toArray();
						
						if(count($payment_office_code)>0){
							$data['ddo'] = $payment_office_code[0]->ddo_code;
						}else{
							$data['ddo'] = '';
						}
  ...
  public function save_payment_details_our_end(Request $request){
		...
		if($input['sId'] == 'S24'){
			...
			$updata =  DB::table('payment_uttarpradesh_ro_mapping')
			->where('dist_no', '=', $input['dist_code'])
			->first();
			
			$division_code = $updata->division_code;
			$treasury_code = $updata->treasury_code;
			
			
			$reff_no = $input['reff_no'];
			$depositor_name = $input['depositor_name'];
			$depositor_address = $input['depositor_address'];
			
		//$rd='http://164.100.180.24/rajkoshlocal/rkapi/rajkoshapi/rajkosh'; 
		//$key = 'Test';
		//$secret='Test123';
		$rd='https://rajkosh.up.nic.in/rkapi/rajkoshapi/Rajkosh'; 
		$key = 'DPS';
		$secret='bVJmO0%GW';
		$amount=$input['amount'];
				
		$dep_code = 		base64_encode('DPS');
		$pmode = 			base64_encode('NETPAY');
		$asmtyear = 		base64_encode('2021-2022');
		$taxperiod = 		base64_encode('O');
		$bank = 			base64_encode('0001');
		$uidno = 			base64_encode($reff_no);
		$div_code = 		base64_encode($division_code);
		$t_code = 			base64_encode($treasury_code);
		$remarks_up=		base64_encode(trim('Payment'));
		$amount_up=			base64_encode(trim($amount));
		$service_id=		base64_encode('01');
		$reff_id=			base64_encode("S01-$reff_no");
		$other1=			strtoupper(hash("SHA512",strtoupper(md5("DPS"."NETPAY"."2021-2022"."O"."$treasury_code"."$amount"))));
		$other2=			strtoupper(hash("SHA512",strtoupper(md5("DPS"."15"."844300121010000"."$amount"))));
		$s_no=				base64_encode('15');
		$head=				base64_encode('844300121010000');
		$name_up	= 		base64_encode(trim($depositor_name));
		$address	= 		base64_encode($depositor_address);
		...
		CURLOPT_POSTFIELDS =>'{"DEPCODE":"'.$dep_code.'","PMODE":"'.$pmode.'","PTYPE":"","CHQDDNO":"","CHQDT":"","ASMTYEAR":"'.$asmtyear.'","TAXPERIOD":"'.$taxperiod.'","PERIOD":"","BANK":"'.$bank.'","UIDNO":"'.$uidno.'","NAME":"'.$name_up.'","DepositorADDRESS":"'.$address.'","DIVCODE":"'.$div_code.'","TCODE":"'.$t_code.'","LOCATION":"","REMARK":"'.$remarks_up.'","TOTAL":"'.$amount_up.'","REFERENCEID":"'.$reff_id.'","ServiceID":"'.$service_id.'","OTHER1":"'.$other1.'","rajkoshHeadInfo":[{"SNO":"'.$s_no.'","HEAD":"'.$head.'","AMOUNT":"'.$amount_up.'","OTHER2":"'.$other2.'"}]}',
		...
		$challan_no = explode("|",$arr[0]->Output1);

		$challan_no = 'DPS'.$challan_no[0];
		$challan_ref_id_date = date('Y-m-d H:i:s',strtotime($challan_no[1]));
		$redirect_url = $arr[0]->Output2;
		...
		$ins =DB::table('payment_details_common')->insert(array(
	    'reff_no'      =>$input['reff_no'], 
	    'dist_code'    =>$input['dist_code'],
	    'st_code'      =>$input['sId'],
	    'pc_no'        =>$input['ac'],
	    'challan_amount'      =>$input['amount'],
	    'challan_ref_id'      =>$challan_no,
	    'challan_ref_id_date'      =>$challan_ref_id_date,
	    'bank_transaction_status' =>0,
	    'candidate_id' =>\Auth::id(), 
  ...
  //dd("conform"); 
	...
	if(count($cntcheck) > 1){
	  Session::flash('status',0);
      Session::flash('flash-message', "Please select same pc nomination.");
      return Redirect::back();
	}
  ...
  $cntcheck=DB::connection('mysql')->table('nomination_application')
   ...
   public function cancel_nomination_prev(Request $request){
	...
	$schedule=DB::connection('mysql')
			...
			->get(); 
	if(count($schedule)>0){
		if((!empty($schedule[0]->st_code)) && (!empty($schedule[0]->pc_no))){	
		$this->update_schedule_status($schedule[0]->st_code, $schedule[0]->pc_no, 2);		
		}
	}	
	////////////////////////////////////////////
	...
	$state = $this->getState($app[0]->st_code); 
	$ac    = $this->getAcName($app[0]->st_code, $app[0]->pc_no); 
  ...
  public function update_schedule_status($st, $ac, $n){
   ...
   public function prev_save(Request $request){
    ...
    $message['Module']= 'SUVIDHA';
    $message['TransectionType']= 'Online Nomination';
    $message['TransectionAction']= 'Appointment date and time for physical submission';
  $get_nominattion_detail = NominationApplicationModel::get_nomination_application($input['nom_id']); 
	    $nomination_num=$get_nominattion_detail['nomination_no'];
	...
	if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Appt Date&time: '.$appdatetime. ' Nomination_Num: '.$nomination_num;
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function delete_draft_nomination(REQUEST $request){ 
   ...
   public function finalize_nomination_payment(REQUEST $request){ 
	...
	if(!isset($input['nom_primary_id'])){
        ...
        $message['MobNo']= $mob[0]->mobile ?? '';
		    $message['applicationType']= 'WebApp';
		    $message['Module']= 'SUVIDHA';
		    $message['TransectionType']= 'Online Nomination';
        $message['TransectionAction']= 'Finalized Nomination';
        if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= ' Nomination Finalized by Candidate ';
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function getAcs(){
	...
	->get(); 
	if(count($nd) > 0 ){
		return $nd[0]->st_code.'***'.$nd[0]->ac_no;
	} 
  }
  ...
  public function nominations(Request $Request){
	...
	if(count($nd) > 0 ){
        ...
        'ac_name' => $this->getAcName($nddata->st_code, $nddata->ac_no),
    ...
    $results = NominationApplicationModel::get_nominations_cust($stdd, $acdd);
	//echo "<pre>"; print_r($results); die;
	$k=1;
    ...
    if(!empty($results)){
	foreach($results as $result){ 
      $encrypt_id = encrypt_String($result['id'].'?acs='.$result['ac_no'].'&std='.$result['st_code']);
      if($result['finalize']){
        $status = 'Finalized';
      }else{
        $status = 'In-completed';
      ...
      $data['results'][] = [
        ...
        'appoinment_scheduled_datetime'       	=>$result['appoinment_scheduled_datetime'],		
        'appoinment_status'       	=>$result['appoinment_status'],
        'view_href'     => url('nomination/detail/'.$encrypt_id.'?acs='.encrypt_String($result['ac_no']).'&std='.encrypt_String($result['st_code'])),
        'edit_href'     => url('nomination/apply-nomination-step-2/'.$encrypt_id),
        'download_href' => url('nomination/download/'.$encrypt_id),
        'is_finalize'   => $result['finalize']
      ];
    ...
    foreach($results_drafts as $rsd){ 
      ...
      $data['redt'][] = [
        ...
        'view_href'     => url('nomination/detail/'.$encrypt_id.'?acs='.encrypt_String($rsd['ac_no']).'&std='.encrypt_String($rsd['st_code'])),
        'edit_href'     => url('nomination/apply-nomination-step-2/'.$encrypt_id),
        'download_href' => url('nomination/download/'.$encrypt_id),
  ...
  public function upload_files(Request $request){
    ...
    $destination_path = 'candprofile/'.$year.'/PC/'.$election_name.'/'.$st_code .'/'. $ac_no;
  ...
  {	
    ...
    $message['Module']= 'SUVIDHA';
    $message['TransectionType']= 'Online Nomination';
    $message['TransectionAction']= ' Payment Pay by Cash/Challan ';
		...
		if(!empty($input['payByCash'])){
			$isInserted =   DB::connection('mysql')->table('challan_payment')
							...
							if($isInserted){
								if(config('public_config.nomination_log'))
				           {
				          $message['LogDescription']= 'Payment Pay By Cash ';
				          $message['TransectionStatus']= 'SUCCESS';
				...
				if($isInserted){
					...
					if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Payment Pay By Challan challan_num' .$input['challan_number'];
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function getStartEndDate(Request $request){
	...
	if($mb =='9899923602' || $mb=='9711225928'){
		return 'Yes***';
	}
		...
		->get();
			if(count($star_end)>0){
				...
				if($start<=$today && $end>$today){
					...
					return 'No***'.$start.'***'.$end.'***'.$this->getAcName($input['sId'], $input['ac']).'***'.$mch;
				}
  ...
  public function isNominationPhysicallySubmitted($no){ 
   ...
   public function getpaymentStatus($nid){
	...
	->whereIn('bank_transaction_status', array('','NULL','1','2','3','4'))
	->orderBy('id','DESC')
	->get();
   }
