...
class xssClean {
    ...
    public function clean_input( $input, $safe_level = 0 ) {
        ...
        $output = str_replace(
                        array('<script>','<scr(script)ipt>','<scr(script)ipt>','<xss>','(alert)','onreadystatechange','iframe'
                        ,'<svg>','embed','meta','string','onload','form','href','onmouseover','onmousemove','marquee onstart','<plaintext\>','</plaintext\>','<svg contentScriptType=text/vbs>','<h1>','</h1>','echo','<FRAMESET>','</FRAMESET>','XSS','xss'
                             ,'xmlns','alert','&','<','>'),
                  '0', $input);
