...
<script type="text/javascript">
  function redirect_parliament(){
    window.location.href = "<?php echo e(config('public_config.ac_url')); ?>";
  }
</script>
               
    <?php if(session('data_username')): ?>
        <div class="alert alert-danger"> <?php echo e(session('data_username')); ?></div>
    <?php endif; ?>
    <span class="help-block"> 
        <strong><?php echo e(Session::get('log_message')); ?></strong>
    </span>
    <?php if(Session::has('flash-message')): ?>
      ...
      <div class="alert <?php echo $class; ?>">
        <?php echo e(Session::get('flash-message')); ?>
...
if(Session::has('DB_id')){
    ...
    <form method="POST" action="<?php echo url('change-database'); ?>" id="change_databsse"> 
      ...
      <div class="form-group">
            <select name="database" class="form-control" id="new" onchange="submit()">
                ...
                <?php if(isset($elec_details)): ?>
                <?php $__currentLoopData = $elec_details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $details): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <option value="<?php echo e($details->id); ?>" <?php if($DB_id == $details->id): ?> selected="selected" <?php endif; ?>  ><?php echo e($details->description); ?></option>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          <?php endif; ?>
        </select>
                 
           <!--    <input type="radio" name="database" value="1" <?php if($cdatabase=='1'): ?> checked="checked" <?php endif; ?>  onclick="change_database()" id="new">  <label id="" for="new"> Current Election  ( BYE) </label>
              <input type="radio" name="database" value="0" <?php if($cdatabase=='0'): ?> checked="checked" <?php endif; ?>  onclick="change_database()"  id="old"> <label for="old"> Previous Election </label> -->
      </div>
    </form>
    <script type="text/javascript">
    ...
    <form class="log-frm-area" id="login_via_ajax" method="POST" action="<?php echo $action; ?>" autocomplete='off' enctype="x-www-urlencoded">
    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>" id="token">
    
      <div class="form-group">
        <input id="username" type="text" class="form-control<?php echo e($errors->has('username') ? ' is-invalid' : ''); ?>" name="username" value="<?php echo e(old('username')); ?>"  autofocus placeholder="User Name"  autocomplete="off" >
        <?php if($errors->has('username')){ ?>
          <span class='invalid-feedback'><strong><?php echo $errors->first('username');; ?></strong></span>
        <?php } ?>
      </div>
      ...
      <?php }else{ ?>
      <div class="form-group"> 
          <input id="password" type="password" class="form-control<?php echo e($errors->has('password') ? ' is-invalid' : ''); ?>" name="password"  placeholder="Password" autocomplete="new-password" autocomplete="off" >
          <?php if($errors->has('password')){ ?>
          <span class='invalid-feedback'><strong><?php echo $errors->first('password');; ?></strong></span>
          <?php } ?>
      </div>
      <?php } ?>
	   ...
	   <div class="form-group  d-flex flex-column flex-md-row align-items-center mb-3">
                    <div class="col col-xs-12 m-0 p-0"> 
						<div class="captcha">
								<span id="captcha"><img id="refresh" src="<?php echo e(captcha_src()); ?>" alt="captcha" class="captcha-img" data-refresh-config="default"></span>
        ...
        <div class="col  pr-0 d-flex align-items-center capchtainpyt">  	
          ...
          <div class="row">
         <input id="lcaptcha" type="text" class=" col-md-7 form-control<?php echo e($errors->has('lcaptcha') ? ' is-invalid' : ''); ?>" name="lcaptcha"  placeholder="captcha"   autocomplete="off"/>&nbsp;
          ...
          <span class='invalid-feedback'><strong><?php echo $errors->first('lcaptcha');; ?></strong></span>
          <?php } ?>
						  </div>
              <div class="col form-group"> 
                <small><a href="<?php echo url('/forgot'); ?>" class="pull-right">Forgot Password</a></small>
              </div>
            </div>
...
<?php endif; ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('admin.layouts.login', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\xampp\htdocs\suvidha\resources\views/admin/auth/web/login.blade.php ENDPATH**/ ?>